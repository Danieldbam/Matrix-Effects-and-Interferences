...
setwd(filepath)
getwd()
filename <- "Skyline-Exp04-PAN-PCI-List-Chromatograms and TIC.tsv"
file <- "Skyline-Exp04-KU-IS-PAN-PCI.tsv"
data <- read_tsv(file)
...
Pressure_data <- read_csv("240815-PAN-001-Blank-1uM-05ulmin-pump pressure.csv", skip = 3)
...
# Save as TIFF
ggsave(
  filename = file.path(output_dir, "Figure 1.tiff"),
  plot = Figure.1,
  width = 20,
  height = 15,
  units = "cm",
...
### Data: faster to extract TIC using OpenMS
 ...
 tic_df <- tic_df %>%
  ...
  mutate(Sample = case_when(
   ...
   mutate(Average = rollmean(intensity, k = 9, fill = NA, align = "left"),
         Average2 = rollmean(intensity, k = 21, fill = NA, align = "left")) %>%
...
wash_labels <- c(
  "PPT" = "Protein precipitation (PPT)",
  "NoWash" = "Phospholipid removal filter (unwashed)",
  "22x" = "Phospholipid removal filter (22x washed)"
)
...
Figure.2 <- tic_df %>% 
  filter(Sample == "NIST",
         ...
         Replicate == 3,
         polarity == "POS") %>% 
  ggplot(aes(x = rt, y = Average, group = Wash, color = Wash)) +
  
  # Shaded regions
  geom_rect(data = data.frame(), aes(xmin = 7.5, xmax = 14, ymin = -Inf, ymax = Inf), 
            fill = "gray80", alpha = 0.5, inherit.aes = FALSE) +
  geom_rect(data = data.frame(), aes(xmin = 5, xmax = 7, ymin = -Inf, ymax = Inf), 
  ...
  # Annotations
  annotate("text", x = 10.75, y = max_intensity * 1.05, 
           label = "Phospholipids", size = 5, fontface = "bold", hjust = 0.5) +
  annotate("text", x = 6, y = max_intensity * 1.05, 
           label = "Filter Contaminants", size = 5, fontface = "bold", hjust = 0.5) +
  ...
  # Custom legend order and labels
  scale_color_brewer(palette = "Dark2", 
                     breaks = wash_order,
                     labels = wash_labels) +
  
  # Y-axis limits
  ylim(0, max_intensity * 1.1) +
  
  # Theme cleanup
  theme_minimal(base_size = 14) +
  theme(
  legend.position = "bottom",         # or "bottom" if preferred
  legend.direction = "vertical",     # stack items vertically
  legend.box = "vertical",           # enforce vertical layout for multiple guides
  panel.grid = element_blank(),
  axis.line = element_line(color = "black"),
  axis.ticks = element_line(color = "black")
) +
guides(color = guide_legend(ncol = 1))  # one column = stacked vertically
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 2.png"),
  plot = Figure.2,
  width = 20,
  height = 15,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 2.tiff"),
  plot = Figure.2,
  width = 20,
  height = 15,
  units = "cm",
...
long_data <- combined_data %>%
  ...
  mutate(Average = rollmean(Intensities, k = 9, fill = NA, align = "left"),
         Average2 = rollmean(Intensities, k = 3, fill = NA, align = "left"))
```
...
Figure.3 <- summary_table_F %>%
 ...
 geom_rect(data = summary_table_F %>% filter(Wash == "PLR unwashed") %>% distinct(Wash, Compound),
  ...
  geom_text(data = summary_table_F %>% filter(Wash == "PPT") %>% distinct(Wash, Compound),
            ...
            aes(x = 10.75, y = max_intensity * 1, label = "Phospholipids"), 
  ...
  geom_text(data = summary_table_F %>% filter(Wash == "PLR unwashed") %>% distinct(Wash, Compound),
            aes(x = 6, y = max_intensity * 1, label = "Filter Contaminants"), 
            size = 4, fontface = "bold", hjust = 0.5, inherit.aes = FALSE) +
...
# Save as PNG
ggsave(
  filename = file.path(output_dir, "Figure 3.png"),
  plot = Figure.3,
  width = 20,
  height = 15,
  units = "cm",
...
# Save as TIFF
ggsave(
  filename = file.path(output_dir, "Figure 3.tiff"),
  plot = Figure.3,
  width = 20,
  height = 15,
  units = "cm",
...
# ---- PLOT C: EIC + Brackets ----
plot_EIC_sig_brackets1 <- ggplot(summary_table_1 %>%
                                   filter(Comparison == "NIST / Solvent",
                                          between(avg_time, zoom_limits[1], zoom_limits[2]))) +
  geom_ribbon(aes(x = avg_time, ymin = Solvent_lower, ymax = Solvent_upper, fill = "Solvent"), alpha = 0.3) +
  geom_ribbon(aes(x = avg_time, ymin = NIST_lower, ymax = NIST_upper, fill = "NIST"), alpha = 0.3) +
  geom_line(aes(x = avg_time, y = Solvent_mean, color = "Solvent"), size = 0.8) +
  geom_line(aes(x = avg_time, y = NIST_mean, color = "NIST"), size = 0.8) +
  #geom_segment(data = sig_regions, aes(x = xmin, xend = xmax, y = y, yend = y), size = 0.8) +
  #geom_text(data = sig_regions, aes(x = (xmin + xmax) / 2, y = y + 0.05 * y, label = "*"),            size = 6, fontface = "bold") +
  geom_vline(xintercept = vline_positions, linetype = "dotted") +
  facet_grid(~ Wash) +
  scale_x_continuous(
  breaks = seq(4, 5, by = 0.25),                  # tick positions
  labels = c("4", "", "4.5", "", "5")         # labels for each tick
) +
  labs(
    x = "Retention Time (mins)",
    y = "Intensity",
...
# ---- PLOT C: EIC + Brackets ----
plot_EIC_sig_brackets2 <- ggplot(summary_table_2 %>%
                                   filter(Comparison == "NIST / Solvent",
                                          between(avg_time, zoom_limits[1], zoom_limits[2]))) +
  geom_ribbon(aes(x = avg_time, ymin = Solvent_mean - t_solvent * Solvent_se,
                  ymax = Solvent_mean + t_solvent * Solvent_se, fill = "Solvent"), alpha = 0.3) +
  geom_ribbon(aes(x = avg_time, ymin = NIST_mean - t_nist * NIST_se,
                  ymax = NIST_mean + t_nist * NIST_se, fill = "NIST"), alpha = 0.3) +
  geom_line(aes(x = avg_time, y = Solvent_mean, color = "Solvent"), size = 0.8) +
  geom_line(aes(x = avg_time, y = NIST_mean, color = "NIST"), size = 0.8) +
  #geom_segment(data = sig_regions_2, aes(x = xmin, xend = xmax, y = y, yend = y), size = 0.8) +
  #geom_text(data = sig_regions_2, aes(x = (xmin + xmax) / 2, y = y + 0.05 * y, label = "*"), size = 6, fontface = "bold") +
  geom_vline(xintercept = vline_positions, linetype = "dotted") +
  facet_grid(~ Wash) +
  scale_x_continuous(
  breaks = seq(4, 5, by = 0.25),                  # tick positions
  ...
  scale_fill_discrete(labels = c("Plasma", "Solvent")) +
scale_color_discrete(labels = c("Plasma", "Solvent")) +
...
plot_matrix_effect_1 <- plot_matrix_effect_1 +
  labs(tag = "A") +
  theme(
    plot.tag = element_text(size = 14, face = "bold"),
    plot.tag.position = c(0.0, 0.99)  # x, y in [0, 1] (left-right, bottom-top)
  )

plot_matrix_effect_zoom_1 <- plot_matrix_effect_zoom_1 +
  labs(tag = "B") +
  theme(plot.tag = element_text(size = 14, face = "bold"),
        plot.tag.position = c(0.00, 1.1))

plot_EIC_sig_brackets1 <- plot_EIC_sig_brackets1 +
  labs(tag = "C") +
  theme(plot.tag = element_text(size = 14, face = "bold"),
        plot.tag.position = c(0.0, 0.99))

plot_matrix_effect_2 <- plot_matrix_effect_2 +
  labs(tag = "D") +
  theme(plot.tag = element_text(size = 14, face = "bold"),
        plot.tag.position = c(-0.02, 0.99))

plot_matrix_effect_zoom_2 <- plot_matrix_effect_zoom_2 +
  labs(tag = "E") +
  theme(plot.tag = element_text(size = 14, face = "bold"),
        plot.tag.position = c(-0.02, 1.1))

plot_EIC_sig_brackets2 <- plot_EIC_sig_brackets2 +
  labs(tag = "F") +
  theme(plot.tag = element_text(size = 14, face = "bold"),
        plot.tag.position = c(-0.02, 0.99))
...
Combined_MEvsEIC <- (MEvsEIC_AllRep | MEvsEIC_SomeRep)

print(Combined_MEvsEIC)
```

### article figure 4 save
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 4.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 4.tiff"),
  plot = Combined_MEvsEIC,
  width = 25,
  height = 20,
  units = "cm",
...
annotated_peaks <- scan787_data %>%
  ...
  filter(
    intensity == max(intensity, na.rm = TRUE) |  # always include the highest
    (lag(intensity, 1, default = 0) < intensity & lead(intensity, 1, default = 0) < intensity)
  ) %>%
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 2 Article.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 2 Article.tiff"),
  plot = contaminent_blanks,
  width = 20,
  height = 10,
  units = "cm",
...
```{r}
filepath <- "C:/Users/DanielMalheiro/Clinical Microbiomics/Teamwebsted - MS-Omics Team Folder/Daniel/PhD/Experiments/Exp7-PCI with Samples"
setwd(filepath)
getwd()
list.files
file1 <- "LC_Final results_ver12-Exp7-PCI.xlsx"
data1<- read_excel(file1, sheet = "Meta")
...
output_file <- "annotated_peak_table_UT.csv"

# Function to run detectHomologues and save results
run_detect_homologues <- function(file) {
  detectHomologues(peak_table2, mz_min = 10, mz_max = 400, 
                   rt_min = 1, rt_max = 900, 
                   ppm_tolerance = 5, 
                   min_series_length = 4,
...
merged_df <- difference_left_join(
  ...
  data_meta %>%
    ...
    select(
      ...
      Polarity, Identifier, "Compound #", SMILES, CSID, "InChI Key"
    ),
...
# Create ggplot
g2 <- ggplot(merged_df, aes(x = rt, y = mz, group = homologue_id)) +
  
  # Scatter points with custom aesthetics
  geom_point(aes(color = homologue_id, 
                 size = homologue_id,
                 shape = homologue_id,
                 alpha = homologue_id)) +

  # Line connections for homologous peaks
  geom_line(data = filter(merged_df, homologue_id != 0),
            aes(group = homologue_id), alpha = 0.4, size = 0.1) +
  # Manual aesthetic scales (mapping factors to colors)
  scale_colour_manual(values = color_map) +
  scale_alpha_manual(values = setNames(rep(0.85, colourCount), homologue_levels)) +
  scale_size_manual(values = setNames(rep(1.5, colourCount), homologue_levels)) +
  scale_shape_manual(values = setNames(rep(19, colourCount), homologue_levels)) +
...
### Data and Plot: Based on molecular formula
...
g2tax <- ggplot(merged_df, aes(x = rt, y = mz, group = homologue_id)) +
  ...
  theme(
    legend.position = "right",
    #legend.justification = c(0.5, 0.5),       # Align legend box to bottom-left corner
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 6.png"),
  plot = g2tax,
  width = 28,
  height = 20,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 6.tiff"),
  plot = g2tax,
  width = 28,
  height = 17,
  units = "cm",
...
# 
# #Classification
# on_List <- purrr::map(InChI_Keys, get_classification)
...
compound_info <- merged_df %>%
  ...
  #filter(!(homologue_id %in% c(3, 6))) %>% #removed those with both POS and NEG in their homologue_id
  #mutate(rt = round(rt, 2)) %>%
  unique() %>%
  na.omit()  # Remove NA values

# Step 2: Select the first 5 columns in data
first_five_cols <- data %>% select(1:5)

# Step 3: Select only columns in data that match the Compound # list
selected_compound_cols <- data %>%
  select(all_of(intersect(names(data), compound_info$`Compound #`)))
...
### final fig heatmap output

```{r}
# Define base file path}
filepath <- "C:/Users/DanielMalheiro/OneDrive - Clinical Microbiomics/Documents/R scripts"

# Create subfolder "PCI ME Figures"
output_dir <- file.path(filepath, "PCI ME Figures")
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# ==== TIFF Export (Analytical Chemistry format) ====
tiff(
  filename = file.path(output_dir, "Figure 7.tiff"),
  width = 28,
  height = 20,
  units = "cm",
  res = 300,
...
png(
  filename = file.path(output_dir, "Figure 7.png"),
  width = 28,
  height = 20,
  units = "cm",
  res = 300
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 8. Article.png"),
  plot = glu_ME_MS,
  width = 20,
  height = 13,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 8. Article.tiff"),
  plot = glu_ME_MS,
  width = 20,
  height = 13,
  units = "cm",
...
# PNG
ggsave(
  filename = file.path(output_dir, "Figure 9. Article.png"),
...
# TIFF
ggsave(
  filename = file.path(output_dir, "Figure 9. Article.tiff"),
  plot = mass_simulation_plot,
  width = 20,
  height = 10,
  units = "cm",
...
file <- "Skyline-PCI-InfusionTest-Exp7-Gradient and Dilution.tsv" 
data <- read_tsv(file)
...
plot_nowash_dilution <- summary_nowash_plot %>%
  ...
  #coord_cartesian(ylim = c(global_min_y, global_max_y)) +
    labs( 
      title = bquote(atop("PLR-filter plasma", "Glucose-"^{13}*"C"[6])),
   x = "Retention Time (min)",
  y = "Matrix Effect (%)") +
  theme_minimal() +
 theme(
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 10. Article.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 10. Article.tiff"),
  plot = plot_ppt_nowash_dilution,
  width = 20,
  height = 10,
  units = "cm",
...
# Set working directory
filepath <- "C:/Users/DanielMalheiro/Downloads/Exp7-Gradient and Dilution/mzML-Pan"
setwd(filepath)

# Load saved spectra
combined_spectra_loaded <- readRDS("combined_mass_spectra_241009_NoWash.rds")
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 11.png"),
  plot = MS_dilution,
  width = 20,
  height = 10,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 11.tiff"),
  plot = MS_dilution,
  width = 20,
  height = 10,
  units = "cm",
...
combined_eic_df_filtered_long <- combined_eic_df_with_shift %>%
  ...
  group_by(Wash, rt, Matrix_Effect_Factor, Sample, Replicate, Intensity_Type, Sequence, Compound) %>%
...
All_plot_EIC_long <- combined_eic_df_filtered_long %>%
  ...
  ggplot() +
  geom_line(aes(x = rt, y = Average_Intensity, color = Intensity_Type, group = Intensity_Type), linewidth = 0.5) +  # Plot for average intensity (primary y-axis)
  geom_line(aes(x = rt, y = Matrix_Effect_Factor * scaling_factor, color = "Matrix Factor"), size = 0.5) +  # Plot MEF over Times (secondary y-axis)
  geom_hline(yintercept = 1.00 * scaling_factor, color = "red", linetype = "dashed") +  # Horizontal line at MEF = 1
  facet_grid(Replicate ~ Compound) +
  scale_y_continuous(
    name = "Intensity",
    sec.axis = sec_axis(trans = ~ . / scaling_factor, name = "Matrix Factor")  # Adding secondary axis for MEF
  ) +
  labs(
 ...
 scale_color_manual(
  values = c("Uncorrected Tryptophan Intensity" = "blue",
...
integrated_data_long_wide_filtered <- integrated_data_long_wide %>%
  ...
  filter(!is.na(Compound),
    !(Intensity_Type == "intensity" & Compound != "Uncorrected")  # Remove all average_intensity except for Normal compound
         ...
         # Step 2: Create the plot with spacing, black border around each bar, and customized appearance
         
barplot_compounds <- ggplot(integrated_data_long_wide_filtered, aes(x = Compound, y = integration_area, fill = Compound, group = Replicate)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), color = "black", linewidth = 0.5) +  # Side-by-side bars with black border
  scale_y_continuous(labels = scientific) +
  facet_wrap(~Wash, ncol = 4) +  # Facet by Wash condition
  labs(
    title = "Comparison of Normal vs Corrected Intensity by PCI-IS",
    x = "PCI Correction",
    y = "Integrated Area",
    fill = "Intensity Type"
  ) + 
  ...
  theme(
    ...
    axis.ticks = element_line(color = "black"),
  )  # Make sure Replicates are readable

# Print the plot
print(barplot_compounds)
...
# Print the plot with error bars and RSD% labels
...
integrated_data_long_wide_filtered <- integrated_data_long_wide %>%
  ...
  mutate(
    ...
    Compound = factor(
      Compound,
      levels = c("Normal", "Tryptophan-d5", "Leucine-d10", "Glucose-13C6")
    )  # Set the order of levels for Compound
  ) %>%
  filter(!is.na(Compound),
    !(Intensity_Type == "intensity" & Compound != "Normal")  # Remove all average_intensity except for Normal compound
  )

# Step 2: Create the plot with boxplot and jitter
boxplot_compounds <- ggplot(integrated_data_long_wide_filtered, aes(x = Wash, y = integration_area, fill = Compound, color = Wash)) +
  geom_boxplot(
    aes(group = Wash), 
    color = "black", 
    outlier.size = NA, 
    ...
    outlier.shape = NA, 
    alpha = 0.6, 
    width = 0.7  # Adjust width of the boxplot
  ) + 
  geom_jitter(
    aes(color = Compound), 
  ...
  facet_wrap(~ Compound, ncol = 4) +  # Facet by Wash condition
  labs(
    title = "Comparison of Normal vs Corrected Intensity by PCI-IS",
    x = "Compound",
    y = "Integrated Area",
...
# global_max <- max(integrated_data_long_wide_filtered$integration_area, na.rm = TRUE)
# 
# # Get all unique Wash levels
# wash_groups <- unique(integrated_data_long_wide_filtered$Wash)
# 
...
# #----------------------------------
# 
# # Compute global max for consistent y-axis limits
# global_max <- max(integrated_data_long_wide_filtered$integration_area, na.rm = TRUE)
# 
# # Get all unique Wash levels
# wash_groups <- unique(integrated_data_long_wide_filtered$Wash)
...
# 
# #----------------------------------
# # Loop through each Wash group
# #----------------------------------
# for (wash in wash_groups) {
#   
#   # Subset data for this Wash
#   filtered_data <- integrated_data_long_wide_filtered %>%
#     filter(Wash == wash)
#   
#   # Get unique compounds
#   compounds <- unique(filtered_data$Compound)
#   
...
#     #----------------------------------
#     
#     # 1️⃣ One-way ANOVA across all compounds
#     anova_res <- filtered_data %>%
#       anova_test(integration_area ~ Compound)
#     
#     # 2️⃣ Pairwise t-tests for all compound pairs (BH correction)
#     pairwise_res <- filtered_data %>%
#       pairwise_t_test(
#         integration_area ~ Compound,
#         p.adjust.method = "BH"
#       )
#     
#     print(paste("Wash group:", wash))
#     print(anova_res)
#     print(pairwise_res)
#     
#     #----------------------------------
#     # Create comparisons for plot
#     #----------------------------------
#     my_comparisons <- combn(compounds, 2, simplify = FALSE)
#     
#     # Assign y-positions dynamically
#     y_positions <- seq(
#       from = global_max * 1.05,
#       to   = global_max * 1.25,
...
#       stat_compare_means(
#         method = "anova",
#         label.y = global_max * 1.35,
#         vjust = -0.2
#       ) +
#       labs(
#         title = paste("Wash:", wash),
#         x = NULL,
#         y = "Integrated Area"
#       ) +
#       scale_y_continuous(
#         limits = c(0, global_max * 1.45),
...
#     filter(Wash == wash)
#   
#   # Get unique compounds in this Wash group
#   compounds <- unique(filtered_data$Compound)
#   
#   if (length(compounds) >= 2 && "Normal" %in% compounds) {
#     reference_compound <- "Normal"
#     
#     # Create only Normal-vs-X comparisons
#     my_comparisons <- setdiff(as.character(compounds), reference_compound) %>%
#       lapply(function(comp) c(reference_compound, comp))
#     
#     # Assign y-positions for comparisons so they don't overlap
#     y_positions <- seq(
#       from = global_max * 1.05,
#       to   = global_max * 1.25,
#       length.out = length(my_comparisons)
...
#       ) +
#       # stat_compare_means(
#       #   label.y = global_max * 1.35,  # keep ANOVA above all t-tests
#       #   method = "anova",
#       #   vjust = -0.2
#       # ) +
#       labs(
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure 12. Article.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure 12. Article.tiff"),
  plot = combined_mef_plot,
  width = 20,
  height = 15,
  units = "cm",
...
raw <- read_delim("testchromatograms2.txt",
...
file.list <- read_excel("TestImportfiles.xlsx") %>%
  select(1:2)  %>%
  mutate(
...
if (file.exists("all_result.rds")) {
  
  message("▶ Loading saved all_result.rds...")
  all_result <- readRDS("all_result.rds")
  ...
  # Save for next time
  saveRDS(all_result, "all_result.rds")
...
val_df   <- split_df %>% filter(Set == "Validation")
...
plot_rsd_hist <- validation_summary %>%
  ...
  dplyr::rename(Uncorrected = Mean_RSD_val_uncorr,
...
# =========================
# Normality check (paired differences)
# =========================
...
if (shapiro_res$p.value > 0.05) {
  cat("Normality check: Differences ARE normally distributed (p =",
      signif(shapiro_res$p.value, 3), ")\n")
  normality_status <- "Normal"
} else {
  cat("Normality check: Differences are NOT normally distributed (p =",
      signif(shapiro_res$p.value, 3), ")\n")
...
pval_text <- paste0(test_type, " p = ",
                    formatC(test_res$p.value, format = "e", digits = 2))


# =========================
# HISTOGRAM (Panel A) with p-value annotation
# =========================
...
val_histo <- ggplot(plot_rsd_hist, aes(x = RSD, fill = Group)) +
  ...
  # Median labels
  annotate("text", x = median_rsd$Uncorrected, y = Inf, vjust = 5, hjust = -0.1,
           label = paste0("Median Uncorrected = ", 
  ...
  annotate("text", x = median_rsd$Corrected, y = Inf, vjust = 3, hjust = -0.1,
           label = paste0("Median Corrected = ", 
                          sprintf("%.1f", median_rsd$Corrected), "%"),
           color = "#1f78b4", size = 4, fontface = "bold") +

  # ⭐ P-value annotation (upper right)
  annotate("text", x = Inf, y = Inf, hjust = 1.1, vjust = 7,
...
ggsave(file.path(output_dir, "Figure_13_Histogram_Only.png"),
       plot = val_histo, width = 20, height = 10,
       units = "cm", dpi = 300)

ggsave(file.path(output_dir, "Figure_13_Histogram_Only.tiff"),
       plot = val_histo, width = 20, height = 10,
       units = "cm", dpi = 300, device = "tiff", compression = "lzw")
```
...
## After figure 13 heapmat figure 14

```{r}
use_log_scale <- T   # set to FALSE to disable log transform


rsd_trainwithuncorr <- split_df %>% 
  filter(Set == "Training") %>% 
  group_by(Feature, PCI, CorrectionType) %>%
  summarise(
    train_rsd = 100 * sd(CorrectedArea, na.rm = TRUE) / mean(CorrectedArea, na.rm = TRUE),
...
uncorrected_carnitine_first <- rsd_trainwithuncorr %>%
  filter(CorrectionType == "uncorrected", PCI == "Carnitine-d9") %>%
  mutate(PCI = "None")               # rename PCI

rsd_trainwithuncorr2 <- rsd_trainwithuncorr %>%
  # remove original “uncorrected + carnitine” row(s)
  filter(!(CorrectionType == "uncorrected")) #%>%
  # bind_rows(uncorrected_carnitine_first)

heatmap_train_mat <- rsd_trainwithuncorr2 %>%
  unite(PCI_Corr, PCI, CorrectionType, sep = " | ") %>%
  select(Feature, PCI_Corr, train_rsd) %>%
  pivot_wider(
    names_from = PCI_Corr,
    values_from = train_rsd
  ) %>%
  as.data.frame()

rownames(heatmap_train_mat) <- heatmap_train_mat$Feature
heatmap_train_mat$Feature <- NULL

# Copy matrix
mat <- heatmap_train_mat

# Apply log10 scaling safely if toggle = TRUE
if (use_log_scale) {
  mat <- log10(mat + 1e-6)   # avoids log(0)
}
...
pheatmap(
  ...
  main = "Training Set Heatmap — RSD by Feature × PCI × CorrectionType",
  #scale = "row"     # ⭐ add this line
)
...
```{r}
use_log_scale <- F   # toggle ON/OFF


val_mat_both <- validation_summary %>%
  select(
    Feature,
    Mean_RSD_val_uncorr,
    Mean_RSD_val_best
...
rownames(val_mat_both) <- val_mat_both$Feature
val_mat_both$Feature <- NULL

mat_both <- val_mat_both

if (use_log_scale) {
  mat_both <- log10(mat_both + 1e-6)
}
...
pheatmap(
  ...
  scale = "column",
  color = colorRampPalette(c("navy", "white", "red"))(100),
  main = if (use_log_scale) {
    "Validation Heatmap — log10(RSD) Corrected vs Uncorrected"
  } else {
    "Validation Heatmap — RSD Corrected vs Uncorrected"
  }
)
...
```{r}
# -------------------------------
# Add validation BEST and UNCORRECTED columns
# -------------------------------

# ================================
# OPTIONAL: Toggle log10 scaling
# ================================
use_log_scale <- T   # <- switch to FALSE for raw scale

# Build 2 validation columns
val_cols <- validation_summary %>%
  select(
    Feature,
    Mean_RSD_val_best,
    Mean_RSD_val_uncorr
  ...
  mutate(
    `Best_PCI | Best_Correct | Validation` = Mean_RSD_val_best,
    `None | uncorrected | Validation` = Mean_RSD_val_uncorr
  ) %>%
  select(
    Feature,
    `Best_PCI | Best_Correct | Validation`,
    `None | uncorrected | Validation`
  )

heatmap_train_mat2 <- heatmap_train_mat %>%
  tibble::rownames_to_column("Feature") %>%   # retains X000xx
  left_join(val_cols, by = "Feature") %>%
...
# Use extended matrix
mat <- heatmap_train_mat2

# Optional log10 scaling
if (use_log_scale) {
  mat <- log10(mat + 1e-6)
}
...
clustering_method = "average",
  ...
  #cutree_rows = 10,
  color = colorRampPalette(c("navy", "white", "red"))(100),
  main = "Training + Validation RSD Heatmap"
)

```

additions to the previous heapmat\

```{r}
# ============================================================
# Adjustable RT bin width (change this!)
# ============================================================
RT_BIN_WIDTH <- 1   # e.g. use 1, 2, 3, 5, 0.5

# ============================================================
# 1. Extract mean retention time for each Feature
# ============================================================
rt_lookup <- result_long %>%
  distinct(Feature, mean_rt) %>%
  group_by(Feature) %>%
  summarise(mean_rt = mean(mean_rt, na.rm = TRUE), .groups = "drop")

# ============================================================
# 2. Align RT to the features in the heatmap matrix
# ============================================================
sorted_rt <- rt_lookup %>%
  right_join(
    data.frame(Feature = rownames(mat)),   # match heatmap rownames
    by = "Feature"
  ) %>%
  arrange(mean_rt)

# ============================================================
# 3. Reorder matrix rows based on retention time
# ============================================================
mat_rt_sorted <- mat[sorted_rt$Feature, , drop = FALSE]

# ============================================================
# 4. Build RT annotation bar (adjustable bins)
# ============================================================
anno_row <- data.frame(Retention_Time = sorted_rt$mean_rt)
rownames(anno_row) <- sorted_rt$Feature

# Create bins based on RT_BIN_WIDTH
anno_row$RT_bin <- cut(
  anno_row$Retention_Time,
  breaks = seq(
    floor(min(anno_row$Retention_Time, na.rm = TRUE)),
    ceiling(max(anno_row$Retention_Time, na.rm = TRUE)),
    by = RT_BIN_WIDTH
  ),
  include.lowest = TRUE,
  right = FALSE
...
# Dynamic colors for RT bins
rt_levels <- levels(anno_row$RT_bin)
rt_colors_discrete <- setNames(
  viridis::viridis(length(rt_levels), option = "H"),
  rt_levels
)

# ============================================================
# 5. Plot RT-sorted heatmap
# ============================================================
pheatmap(
  mat_rt_sorted,
  cluster_rows = F,
  cluster_cols = T,
  scale = "row",
  annotation_row = data.frame(RT_bin = anno_row$RT_bin,
                            row.names = rownames(anno_row)),
  annotation_colors = list(RT_bin = rt_colors_discrete),
  color = colorRampPalette(c("navy", "white", "red"))(100),
  fontsize_row = 6,
  fontsize_col = 8,
  show_rownames = FALSE,
  main = paste0("Heatmap Sorted by Retention Time (", RT_BIN_WIDTH, "-minute RT Bins)")
)

```

### Sanity Check for C labeled Leucine
...
# # List of mzML files
# files <- c(
#   "241008-Exp-PAN-1-008-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-009-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-010-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-011-22x-Blank.mzML",
#   "241008-Exp-PAN-1-012-22x-Blank.mzML",
#   "241008-Exp-PAN-1-013-22x-Blank.mzML",
#   "241008-Exp-PAN-1-014-22x-NIST.mzML",
#   "241008-Exp-PAN-1-015-22x-NIST.mzML",
#   "241008-Exp-PAN-1-016-22x-NIST.mzML",
#   "241008-Exp-PAN-1-017-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-018-08x-Blank.mzML",
#   "241008-Exp-PAN-1-019-08x-Blank.mzML",
#   "241008-Exp-PAN-1-020-08x-Blank.mzML",
#   "241008-Exp-PAN-1-021-08x-NIST.mzML",
#   "241008-Exp-PAN-1-022-08x-NIST.mzML",
#   "241008-Exp-PAN-1-023-08x-NIST.mzML",
#   "241008-Exp-PAN-1-024-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-025-04x-Blank.mzML",
#   "241008-Exp-PAN-1-026-04x-Blank.mzML",
#   "241008-Exp-PAN-1-027-04x-Blank.mzML",
#   "241008-Exp-PAN-1-028-04x-NIST.mzML",
#   "241008-Exp-PAN-1-029-04x-NIST.mzML",
#   "241008-Exp-PAN-1-030-04x-NIST.mzML",
#   "241008-Exp-PAN-1-031-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-032-NoWash-Blank.mzML",
#   "241008-Exp-PAN-1-033-NoWash-Blank.mzML",
#   "241008-Exp-PAN-1-034-NoWash-Blank.mzML",
#   "241008-Exp-PAN-1-035-NoWash-NIST.mzML",
#   "241008-Exp-PAN-1-036-NoWash-NIST.mzML",
#   "241008-Exp-PAN-1-037-NoWash-NIST.mzML",
#   "241008-Exp-PAN-1-038-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-039-PPT-Blank.mzML",
#   "241008-Exp-PAN-1-040-PPT-Blank.mzML",
#   "241008-Exp-PAN-1-041-PPT-Blank.mzML",
#   "241008-Exp-PAN-1-042-PPT-NIST.mzML",
#   "241008-Exp-PAN-1-043-PPT-NIST.mzML",
#   "241008-Exp-PAN-1-044-PPT-NIST.mzML",
#   "241008-Exp-PAN-1-045-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-046-22x-NIST.mzML",
#   "241008-Exp-PAN-1-047-22x-NIST.mzML",
#   "241008-Exp-PAN-1-048-22x-NIST.mzML",
#   "241008-Exp-PAN-1-049-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-050-NoFilter-Blank.mzML",
#   "241008-Exp-PAN-1-051-NoFilter-Blank.mzML"
# )
# 
# ppm <- 3
# target_mz <-  #166.0863 # Phenylalanine
#               138.1220  # IsoLeucineC13 m/z value
# 
# # Extract EICs for all files
# eic_list_LeucineClabel <- lapply(files, function(file_name) {
#   extract_eic(file_name, target_mz, ppm)
# })
# 
# filepath <- "C:/Users/DanielMalheiro/OneDrive - Clinical Microbiomics/Documents/R scripts"
# setwd(filepath)
# 
# saveRDS(eic_list_LeucineClabel , file = "eic_list_LeucineClabel.rds")
```

Load and extract PCI of Leucine, Glucose, Glucose
...
eic_list_LeucineClabel <- readRDS("eic_list_LeucineClabel.rds")

# Combine all EICs into a single data frame
combined_eic_df <- bind_rows(eic_list_LeucineClabel)
...
combined_eic_df_filtered %>%
  ...
  ggplot(aes(x = rt, y = intensity, color = Wash)) +
  geom_line(linewidth = 1) +
  facet_grid( ~Wash ~Replicate ~Sample,
              #ncol = 3
              ) 
  labs(
    title = paste("Overlayed Normalized EICs (", ppm, "ppm, Positive Mode)"),
...
filtered_data_2 <- long_data %>%
  filter(
    ...
    Wash %in% c(
      ...
      "04x",
      "NoWash",
      "PPT"),  # Filter for Wash conditions
    !(Replicate %in% c("1", "2", "3", "4", "6") & Sample == "Solvent"),
    Replicate %in% 1:8
  ) %>%
  mutate(Wash = factor(Wash, levels = c("22x","08x","04x","NoWash", "PPT")))
...
matrix_effect_factor_time_plot <- ggplot(matrix_effect_data, aes(x = Times, y = Matrix_Effect_Factor, color = Wash)) +
  geom_line() +
  geom_vline(xintercept = 2.8, linetype = "dotted", color = "black" ) +
 geom_hline(yintercept = 1.00, linetype = "dashed", color = "black" ) +
  facet_grid( ~Wash + Replicate ~Compound ) +
  labs(
    title = "Matrix Effect Factor (MEF) over Time for NIST Replicates",
...
combined_eic_df_filtered_long <- combined_eic_df_with_shift %>%
  ...
  summarise(Average_Intensity = mean(Intensity_Value, na.rm = TRUE), .groups = "drop") %>%
  mutate(
    Compound = factor(Compound, levels = c("Average", "Tryptophan-d5", "Leucine-d10", "Glucose-13C6", "GlutamicAcid-d5"))  # Set the order of levels for Compound
  )

# Calculate scaling factor for Matrix Effect Factor (MEF) to fit on the secondary axis
scaling_factor <- max(
  combined_eic_df_filtered_long$Average_Intensity[combined_eic_df_filtered_long$rt >= 2.5 & combined_eic_df_filtered_long$rt <= 4],
  na.rm = TRUE
) /
max(
  combined_eic_df_filtered_long$Matrix_Effect_Factor[combined_eic_df_filtered_long$rt >= 2.5 & combined_eic_df_filtered_long$rt <= 4],
  na.rm = TRUE
)
...
# Step 7: Plot with alignment by rt and Times
All_plot_EIC_long <- combined_eic_df_filtered_long %>%
  filter(between(rt, 2, 4),
         Sample == "NIST",
         Compound == "Leucine-d10",
         Wash %in% c("PLR Unwashed","PLR 4x Wash"),
         Replicate %in% 1:3) %>%
  ggplot() +
  geom_line(aes(x = rt, y = Average_Intensity, color = Intensity_Type, group = Intensity_Type), linewidth = 0.5) +  # Plot for average intensity (primary y-axis)
  geom_line(aes(x = rt, y = Matrix_Effect_Factor * scaling_factor, color = "MEF"), size = 0.5) +  # Plot MEF over Times (secondary y-axis)
  geom_hline(yintercept = 1.00 * scaling_factor, color = "red", linetype = "dashed") +  # Horizontal line at MEF = 1
  facet_grid(Replicate ~ Sample ~ Compound ~Wash) +
  scale_y_continuous(
    name = "Intensity",
    sec.axis = sec_axis(trans = ~ . / scaling_factor, name = "Matrix Effect Factor")  # Adding secondary axis for MEF
  ) +
  labs(
    title = "Overlayed Normalized EICs with Matrix Effect Factor",
    x = "Retention Time (min)",
  ...
  scale_color_manual(
    values = c("MEF" = "red", 
               "average_intensity" = "blue", 
               "corrected_intensity" = "green"),
    labels = c("average_intensity" = "Uncorrected Tryptophan Intensity", 
               "corrected_intensity" = "Corrected Tryptophan Intensity", 
               "MEF" = "Matrix Effect Factor")  # Customize legend labels
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
integrated_data_long_wide_filtered <- integrated_data_long_wide %>%
  group_by(Wash, Replicate, Compound) %>%
  filter(!(Intensity_Type == "average_intensity" & duplicated(Replicate))) %>%  # Remove duplicates of average_intensity for each replicate
  mutate(
    # Create a combined Intensity_Type with Compound to differentiate the intensity types by compound
    Intensity_Compound = paste(Intensity_Type, Compound, sep = "_"),
    Compound = if_else(
    ...
    Compound = factor(
      Compound,
      levels = c("Uncorrected", "Leucine-d10","Tryptophan-d5", "Glucose-13C6", "GlutamicAcid-d5")
    )  # Set the order of levels for Compound
  ) %>%
  filter(!is.na(Compound),
    !(Intensity_Type == "average_intensity" & Compound != "Uncorrected")  # Remove all average_intensity except for Normal compound
  )


         # Step 2: Create the plot with spacing, black border around each bar, and customized appearance
         
barplot_compounds <- ggplot(
  integrated_data_long_wide_filtered,
  aes(
    x = Compound,
    y = integration_area,
    fill = Compound,
  ...
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), color = "black", linewidth = 0.5) +  # Side-by-side bars with black border
  scale_y_continuous(labels = scientific) +
  facet_wrap(~Wash  ~Sample ,
             #ncol = 4
             ) +  # Facet by Wash condition
  labs(
    title = "Comparison of Normal vs Corrected Intensity by PCI-IS",
    x = "PCI Correction",
    y = "Integrated Area",
...
# Step 1: Calculate the standard deviation (SD), standard error (SE), and RSD% for each Wash and Compound
error_data <- integrated_data_long_wide_filtered %>%
  filter(!(Intensity_Type == "intensity") | Compound == "Uncorrected") %>%
  group_by(Wash, Sample, Compound, Intensity_Type) %>%
  summarise(
    Mean = mean(integration_area, na.rm = TRUE),
    SD = sd(integration_area, na.rm = TRUE),
    SE = SD / sqrt(n()),  # Calculate standard error
...
# Step 2: Create the plot with bar chart, error bars, and RSD% labels
barplot_compounds_with_error <- error_data %>%
  #filter(Compound %in% c("Uncorrected", "Leucine-d10")) %>%
  filter(Sample == "NIST") %>%
  ggplot(aes(x = Wash, y = Mean, fill = Compound)) +
  # Create the bars
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), color = "black", linewidth = 0.5) + 
  # Add error bars
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), 
...
barplot_compounds_with_error <- error_data %>%
  filter(
    Compound == "Uncorrected" |
    (Compound %in% c("Leucine-d10", "Tryptophan-d5") & 
     Wash %in% c("PLR 4x Wash", "PLR Unwashed"))
  ) %>%
  filter(Sample == "NIST") %>%
...
### SI.Fgure 2: MS of contamients as image

## SI-Figure 4. Over laps of cotaminant ions and ME
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "SI_Figure 3 Article.png"),
  plot = pub_ready_plot,
  width = 20,
  height = 10,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "SI_Figure 3 Article.tiff"),
  plot = pub_ready_plot,
  width = 20,
  height = 10,
  units = "cm",
...
## SI-Figure 6. EIC of Acetyl-Carnitine and Glucose-13C6
...
chromatogram_overlay_scaled <- combined_eic_df_scaled %>%
  ...
  ggplot(aes(x = rt, y = scaled_intensity, color = target)) +
...
## SI-Figure 5 Injecion time plot
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "SI-Figure 5. Article.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "SI-Figure 5. Article.tiff"),
  plot = Ioninjection_plot,
  width = 20,
  height = 10,
  units = "cm",
...
## plotting single MF and peak

```{r}
# Step 7: Plot with alignment by rt and Times
single_plot_EIC_long <- combined_eic_df_filtered_long %>%
  filter(between(rt, 4.5, 5.1),
         Wash == "PLR 4x Wash",
         Replicate == 2,
  ...
  geom_line(aes(x = rt, y = Matrix_Effect_Factor * scaling_factor, color = "MEF"), size = 0.5) +  # Plot MEF over Times (secondary y-axis)
  #geom_hline(yintercept = 1.00 * scaling_factor, color = "red", linetype = "dashed") +  # Horizontal line at MEF = 1
  geom_vline(xintercept = c(4.8, 5),  color = "black", linetype = "dashed") +  # Horizontal line at MEF = 1
  facet_grid(Replicate ~ Compound) +
  scale_y_continuous(
    name = "Intensity",
    sec.axis = sec_axis(trans = ~ . / scaling_factor, name = "Matrix Factor")  # Adding secondary axis for MEF
  ...
  scale_color_manual(
    ...
    labels = c("average_intensity" = "Uncorrected Tryptophan Intensity", 
               "corrected_intensity" = "Corrected Tryptophan Intensity", 
               "MEF" = "Matrix Factor")  # Customize legend labels
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
# Print the plot
print(single_plot_EIC_long)
```

ploting single

```{r}
# Compute MF stats in the 4.8–5.0 window
mf_stats <- combined_eic_df_filtered_long %>%
  filter(between(rt, 4.8, 5.0),
         Wash == "PLR 4x Wash",
         Replicate == 3,
         Compound == "Tryptophan-d5",
         Intensity_Type == "intensity") %>%
  summarise(
    mf_mean   = mean(Matrix_Effect_Factor, na.rm = TRUE),
    mf_median = median(Matrix_Effect_Factor, na.rm = TRUE),
    mf_min    = min(Matrix_Effect_Factor, na.rm = TRUE)
  )

# Extract as numeric and scale to match primary y-axis
mf_mean_y   <- mf_stats$mf_mean   * scaling_factor
mf_median_y <- mf_stats$mf_median * scaling_factor
mf_min_y    <- mf_stats$mf_min    * scaling_factor
...
single_plot_EIC_long <- combined_eic_df_filtered_long %>%
  filter(between(rt, 4.5, 5.1),
         ...
         Intensity_Type == "intensity") %>%
  ggplot() +
  geom_line(aes(x = rt, y = Average_Intensity, color = Intensity_Type, group = Intensity_Type), linewidth = 0.5) +
  geom_line(aes(x = rt, y = Matrix_Effect_Factor * scaling_factor, color = "MEF"), linewidth = 0.5) +

  # Vertical guides at 4.8 and 5.0
  geom_vline(xintercept = c(4.8, 5.0), color = "black", linetype = "dashed") +

  # Horizontal lines for MF stats (scaled to primary axis)
  geom_hline(yintercept = mf_mean_y,   linetype = "solid",  linewidth = 0.4, color = "blue") +
  geom_hline(yintercept = mf_median_y, linetype = "dashed", linewidth = 0.4, color = "blue") +
  geom_hline(yintercept = mf_min_y,    linetype = "dotted", linewidth = 0.4, color = "blue") +

  facet_grid(Replicate ~ Compound) +
  scale_y_continuous(
    name = "Intensity",
    sec.axis = sec_axis(trans = ~ . / scaling_factor, name = "Matrix Factor")
  ) +
  labs(
    title = "Overlayed Normalized EICs with Matrix Effect Factor",
    x = "Retention Time (min)",
...
# (Optional) quick readout of the MF stats for the plotted window
mf_stats

```

```{r}
# If you ever compute SE later, add it here. For now assume SE = 0
rsd_train.tryp <- rsd_train.tryp %>%
  mutate(SE = 0)      # placeholder so geom_errorbar works

barplot_rsd <- ggplot(rsd_train.tryp, aes(x = PCI, y = train_rsd, fill = PCI)) +
  ...
  # ---- Error bars (currently zero-height) ----
  #geom_errorbar(aes(ymin = train_rsd - SE, ymax = train_rsd + SE),
                width = 0.25,
                position = position_dodge(width = 0.9),
                color = "black") +
  ...
  # ---- RSD% labels ----
  geom_text(
    aes(y = train_rsd + SE + (0.05 * max(train_rsd))), 
    label = sprintf("%.1f%%", train_rsd), 
    position = position_dodge(width = 0.9),
    size = 3,
    vjust = -0.3,
    color = "black"
  ...
  # ---- Facet by CorrectionType ----
  facet_wrap(~ CorrectionType, ncol = 2) +
  
  # ---- Axes and labels ----
  labs(
    title = paste0("RSD (%) for ", feature, " by PCI and Correction Type"),
    y = "RSD (%)",
    x = NULL
  ) +
