---
title: "Reproducibility and Code Availability for PCI Analysis"
format: html
editor: visual
editor_options: 
...
small_data <- long_data %>%
  ...
  group_by(Flow_Rate) %>%
  mutate(
    Average = rollmean(Intensities, k = 3
                       , fill = "extend", align = "left"),
...
tic_df <- tic_df_all %>%
 ...
 tic_df <- tic_df %>%
  ...
  mutate(Sample = case_when(
   ...
   mutate(Average = rollmean(intensity, k = 3, fill = NA, align = "left")) %>%
...
long_data <- combined_data %>%
  ...
  mutate(Average = rollmean(Intensities, k = 3, fill = NA, align = "left"))
```
...
summary_table_F <- filtered_data %>%
  ...
  mutate(
    Wash = recode(Wash, "NoWash" = "PLR unwashed","22x" = "PLR 22x wash"),
    Wash = factor(Wash, levels = c("PPT", "PLR unwashed","PLR 22x wash")),
...
# Plot
# ===============================
Figure.3 <- ggplot(summary_table_F, aes(x = avg_time)) +
  ...
  # Late elution shading (ALL panels)
  annotate(
    "rect",
    xmin = 7.5, xmax = 14,
    ymin = -Inf, ymax = Inf,
  ...
  geom_text(
    data = annot_PLR,
    aes(x = 6, y = max_intensity * 1.10, label = "Filter Contaminants"),
    size = 4,
    fontface = "bold",
    inherit.aes = FALSE
  ) +
...
# ---- PLOT A: Full Matrix Effect, selected blanks ----
...
# ---- PLOT B: Zoomed-In Matrix Effect, selected blanks ----
...
# ---- PLOT C: EIC + Brackets, selected blanks ----
...
# ============================
# 2. Gaussian Simulation
# ============================

gauss <- function(m, I, R) {
  fwhm <- m / R
  sigma <- fwhm / 2.35482
  I * exp(-(x_vals - m)^2 / (2 * sigma^2))
}

sim_60k <- data.frame(
  mz = x_vals,
  peak_204_1173 = gauss(mz1, int1, 60000),
  peak_204_1230 = gauss(mz2, int2, 60000),
  Resolution = "60k"
)

sim_120k <- data.frame(
  mz = x_vals,
  peak_204_1173 = gauss(mz1, int1, 120000),
  peak_204_1230 = gauss(mz2, int2, 120000),
  Resolution = "120k"
)

sim_all <- bind_rows(sim_60k, sim_120k) %>%
  mutate(combined = peak_204_1173 + peak_204_1230) %>%
...
calc_peak <- function(mz, int) {
  peak_max <- max(int)
  half <- baseline + (peak_max - baseline) / 2
  apex <- which.max(int)
  
  left <- right <- NA
  
  for (i in apex:2) {
    if (int[i] >= half && int[i-1] < half) {
      left <- mz[i-1] + (half - int[i-1]) *
        (mz[i] - mz[i-1]) / (int[i] - int[i-1])
      break
    }
  }
  
  for (i in apex:(length(mz)-1)) {
    if (int[i] >= half && int[i+1] < half) {
      right <- mz[i] + (half - int[i]) *
        (mz[i+1] - mz[i]) / (int[i+1] - int[i])
      break
    }
  }
...
get_res <- function(df, label) {
  mz <- df$mz
  
  p1 <- calc_peak(mz, df$peak_204_1173)
  p2 <- calc_peak(mz, df$peak_204_1230)
  
  data.frame(
    mz = c(mz1, mz2),
    y = c(p1[1], p2[1]),
    Resolution_val = c(mz1 / p1[2], mz2 / p2[2]),
    Setting = label
  )
}
...
res_all <- bind_rows(
  get_res(sim_60k, "60k"),
  get_res(sim_120k, "120k")
)

# --- experimental

get_exp <- function(mz, int, center) {
  idx <- mz >= (center - 0.01) & mz <= (center + 0.01)
  calc_peak(mz[idx], int[idx])
}

m1 <- get_exp(mass_spectrum_4$mz, mass_spectrum_4$intensity, mz1)
m2 <- get_exp(mass_spectrum_4$mz, mass_spectrum_4$intensity, mz2)

res_exp <- data.frame(
  mz = c(mz1, mz2),
  y = c(m1[1], m2[1]),
  Resolution_val = c(mz1 / m1[2], mz2 / m2[2]),
  Setting = "Experimental"
)

# combine + format + ordering + filtering
res_all <- bind_rows(res_all, res_exp) %>%
  mutate(
    Resolution_val = round(Resolution_val),
    Resolution = recode(Setting,
    ...
    label = paste0("R = ", Resolution_val)
  ) %>%
filter(!(mz == mz1 & Resolution %in% c("Experimental Data", "60k Resolving Power")))
...
mass_simulation_plot <- combined_simulation %>%
  ...
  geom_vline(xintercept = c(mz1, mz2), linetype = "dashed", color = "black") +
...
extract_spectra_from_file <- function(file_path, rt_targets, scan_labels, mz_range) {
  ...
  spectra <- purrr::map2_dfr(seq_along(selected_scans), selected_scans, function(i, scan_index) {
...
cache_file1 <- "combined_mass_spectra_241009_Nowash_Glucose_13C6.rds"

if (file.exists(cache_file1)) {
  
  message("Loading cached spectra from: ", cache_file1)
  glucose_nowash_spectra <- readRDS(cache_file1)
  
} else {
  
  message("Cached spectra not found. Extracting spectra...")
 ...
 saveRDS(
  glucose_nowash_spectra,
  here("Data", "combined_mass_spectra_241009_Nowash_Glucose_13C6.rds")
)
  
  message("Saved extracted spectra to: ", cache_file1)
}
...
MS_glucose_nowash_dilution <- glucose_nowash_spectra %>%
  filter(Replicate == 2) %>%
  ggplot(aes(x = mz, y = intensity)) +
  geom_line(linewidth = 1, aes(color = scan)) +
  geom_vline(
  ...
  labs(
  title = bquote(atop("PLR unwashed plasma",
                      "Glucose -"^{13}*"C"[6])),
  y = "Intensity"
) +
theme_minimal(base_size = 14) +
...
# ============================================================
# Helper: identify positive scans
# Works for polarity stored as 1, "+", "positive", "Positive", etc.
# ============================================================
...
cache_file2 <- "combined_mass_spectra_241009_PPT_Leucine_d10.rds"

if (file.exists(cache_file2)) {
  
  message("Loading cached spectra from: ", cache_file2)
  leucine_ppt_spectra <- readRDS(cache_file2)
  ...
  saveRDS(
  leucine_ppt_spectra,
  here("Data", "combined_mass_spectra_241009_PPT_Leucine_d10.rds")
)
  
  message("Saved extracted spectra to: ", cache_file2)
}
...
MS_leucine_ppt_dilution <- leucine_ppt_spectra %>%
  filter(Replicate == 2) %>%
  ggplot(aes(x = mz, y = intensity)) +
  geom_line(linewidth = 1, aes(color = scan)) +
  geom_vline(
...
# Save results
saveRDS(eic_list, file = (here("Data","eic_list.rds")))
...
debugging figure glutamic acid

```{r}
# ============================================================
# Debug why glutamic acid is missing
# Correctly uses glutamic acid RT from RT_interval, not feature_info
# ============================================================

ppm <- 5
rt_tol <- 0.1

debug_name <- "L-Glutamic acid"
debug_mz <- 148.0604

# -----------------------------
# Find glutamic acid-like features in feature_info
# -----------------------------

feature_info_glutamic <- feature_info %>%
  ungroup() %>%
  filter(str_detect(Name, regex("glutamic", ignore_case = TRUE))) %>%
  select(Feature, Name, mz, rt) %>%
  distinct()

cat("\nFeatures in feature_info containing 'glutamic':\n")
print(feature_info_glutamic)

# -----------------------------
# Find all RT_interval rows matching glutamic acid m/z
# -----------------------------

ppm_tol  <- debug_mz * ppm / 1e6
mz_lower <- debug_mz - ppm_tol
mz_upper <- debug_mz + ppm_tol

glutamic_mz_matches <- RT_interval %>%
  ...
  mutate(
    ...
    target_mz = debug_mz,
    mz_error_ppm = (`m/z` - debug_mz) / debug_mz * 1e6
  ) %>%
  arrange(`RT [min]`)

cat("\nRows matching glutamic acid m/z only:\n")
print(
  glutamic_mz_matches %>%
    select(`Study File ID`, `m/z`, `RT [min]`, mz_error_ppm)
)

# -----------------------------
# Use median observed RT from RT_interval
# -----------------------------

debug_rt <- median(glutamic_mz_matches$`RT [min]`, na.rm = TRUE)

cat("\nUsing observed glutamic acid RT from RT_interval:", debug_rt, "\n")
cat("m/z window:", mz_lower, "-", mz_upper, "\n")
cat("RT window:", debug_rt - rt_tol, "-", debug_rt + rt_tol, "\n")

# -----------------------------
# Final matches using correct m/z and RT
# -----------------------------

glutamic_matches <- RT_interval %>%
  mutate(`RT [min]` = as.numeric(`RT [min]`)) %>%
  filter(
    between(`m/z`, mz_lower, mz_upper),
    between(`RT [min]`, debug_rt - rt_tol, debug_rt + rt_tol)
  ) %>%
  inner_join(
    file.list %>%
  ...
  mutate(
    ...
    target_rt = debug_rt,
    mz_error_ppm = (`m/z` - debug_mz) / debug_mz * 1e6,
    rt_error_min = `RT [min]` - debug_rt,
    abs_rt_error = abs(`RT [min]` - debug_rt)
  ) %>%
  group_by(`Study File ID`) %>%
  slice_min(abs_rt_error, n = 1, with_ties = FALSE) %>%
  ungroup()

cat("\nFinal counted files for glutamic acid:", n_distinct(glutamic_matches$`Study File ID`), "\n")
...
glutamic_summary <- glutamic_matches %>%
  summarise(
    target_name = first(target_name),
    target_mz = first(target_mz),
    target_rt = first(target_rt),
    n_files_found = n_distinct(`Study File ID`),
    files_found = paste(unique(`Study File ID`), collapse = "; ")
  )

print(glutamic_summary)
```
...
rsd_train_all <- train_df %>%
  ...
  summarise(
    Best_train_RSD = 100 * sd(CorrectedArea, na.rm = TRUE) /
      mean(CorrectedArea, na.rm = TRUE),
    .groups = "drop"
  )

# ---------------------------
# 2. Training RSDs for uncorrected data
# ---------------------------
...
train_uncorr_rsd <- uncorrected_df %>%
  ...
  group_by(Feature) %>%
  summarise(
    train_rsd_uncorrected = 100 * sd(CorrectedArea, na.rm = TRUE) /
      mean(CorrectedArea, na.rm = TRUE),
    .groups = "drop"
  )

# ---------------------------
# 3. Best PCI per feature for each correction type
# ---------------------------
...
# ---------------------------
# 4. Validation RSD for each correction using selected best PCI
# ---------------------------
...
rsd_val_by_correction <- val_df %>%
  ...
  summarise(
    RSD_corrected = 100 * sd(CorrectedArea, na.rm = TRUE) /
      mean(CorrectedArea, na.rm = TRUE),
    .groups = "drop"
  )

# ---------------------------
# 5. Validation uncorrected RSD for all features
# ---------------------------

uncorr_val <- validation_summary %>%
  ungroup() %>%
  select(Feature, Mean_RSD_val_uncorr) %>%
  distinct() %>%
  dplyr::rename(
    RSD_uncorrected = Mean_RSD_val_uncorr
  )

# ---------------------------
# 6. Choose corrected vs uncorrected
# based on TRAINING RSD
# but keep validation RSDs
# ---------------------------

best_vs_uncorrected <- uncorr_val %>%
  left_join(
  ...
  mutate(
    Training_uncorrected_is_less = train_rsd_uncorrected <= Best_train_RSD,
...
```{r}
file_TLextend <- "C:/Users/DanielMalheiro/Clinical Microbiomics/Teamwebsted - MS-Omics Team Folder/Daniel/PhD/Experiments/Exp7-PCI with Samples/Skyline-PCI-Exp7-QCcheck-241008-TLextend.tsv" 
data_neg <- read_tsv(here(file_TLextend))
replacements <- c(
  "Leucine d10" = "Leucine-d10" ,
   "Glutamic acid-d5"= "GlutamicAcid-d5",
   "DL-Tryptophan-d5" = "Tryptophan-d5",
...
data_neg2 <- data_neg2 %>%
  bind_rows(
    data_neg2 %>%
      filter(Sample == "Reference") %>%
      select(-Wash) %>%  # Remove the existing Wash column
      crossing(Wash = unique(data_neg2$Wash[data2$Wash != "NoFilter"]))  # Add each wash except "NoFilter"
  ) %>%
filter(Wash != "NoFilter") %>%
filter(Wash %in% c("NoFilter", "22x", "08x", "04x","NoWash","PPT")) %>%
   mutate(
...
unique_washes <- unique(data_neg2$Wash) %>% print()
unique_sample <- unique(data_neg2$Sample) %>% print()

data_intensity2 <- data_neg2 %>%
  select(Chromatograms, Compound, Intensities) %>%
  distinct() %>%
...
long_data2 <- combined_data2 %>%
  ...
  mutate(
    Average = zoo::rollmean(Intensities, k = 3, fill = NA, align = "left")
  ) %>%
  ungroup()
```
...
# file.remove(here("Data", "all_result_neg.rds"))
...
best_vs_uncorrected_neg <- uncorr_val_neg %>%
  left_join(
...
uncorrected_plot_neg <- uncorr_val_neg %>%
  left_join(
    train_uncorr_rsd_neg,
    by = "Feature"
  ) %>%
...
rsd_train.tryp <- split_df %>% 
  #filter(Set == "Validation") %>% 
  group_by(Feature, PCI, CorrectionType, Set) %>%
  summarise(
...
plot.tryp.test.rsd <- ggplot(rsd_train.tryp, aes(x = PCI, y = train_rsd, fill = PCI)) +
  ...
  facet_wrap(~Set ~CorrectionType , nrow = 1, strip.position = "top") +
...
## Figure SI-4. Over laps of contaminant ions and ME
...
average_nist_22x <- summed_signals %>%
  ...
  mutate(
    rolling_intensity = rollmean(total_intensity, k = 3, fill = NA, align = "right")
  )
...
## Figure SI-5. Effect of PLR wash intensity on contaminant signals
...
## Figure SI-6: Injection time plot
...
ggsave(
  filename = file.path(output_dir, "Figure_SI-8_mean_correction_POS_NEG.png"),
  plot = plot_all,
  width = 24,
  height = 18,
  units = "cm",
...
ggsave(
  filename = file.path(output_dir, "Figure_SI-8_mean_correction_POS_NEG.tiff"),
  plot = plot_all,
  width = 24,
  height = 18,
  units = "cm",
...
ggsave(
  filename = file.path(
    output_dir,
    "Figure_SI-9_mean_worsened_POS_NEG.png"
  ),
  plot = WC_plot,
  width = 22,
  height = 30,
...
ggsave(
  filename = file.path(
    output_dir,
    "Figure_SI-9_mean_worsened_POS_NEG.tiff"
  ),
  plot = WC_plot,
  width = 22,
  height = 30,
...
# Filter only 60K
#---------------------------------------------
combined_spectra_60K <- combined_spectra %>%
  filter(sample == "60K") %>%
...
# Plot
#---------------------------------------------
spectrum_glucose <- ggplot(
  combined_spectra_60K,
  ...
  scale_color_manual(
    values = c(
      "Scan 1" = "#5AAA46",  # main green
      "Scan 2" = "#8BCF7A",  # lighter green
      "Scan 3" = "#D96B63",  # lighter red
      "Scan 4" = "#C03830"   # main red
    )
  ) +
...
#---------------------------------------------
ggsave(
  filename = "spectrum_glucose_60K.png",
...
```{r}
file <- "Skyline-PCI-Exp7-QCcheck-241008-TLextend.tsv" 
data <- read_tsv(here("Data", file))
replacements <- c(
  "Leucine d10" = "Leucine-d10" ,
   "Glutamic acid-d5"= "GlutamicAcid-d5",
...
data_intensity <- data2 %>%
  ...
  mutate(across(starts_with("Intensities"),
                ~ as.numeric(gsub(",", ".", .))))
...
data_time <- data2 %>%
  ...
  mutate(across(starts_with("Times"),
                ~ as.numeric(gsub(",", ".", .))))

# --- Combine safely (NO join) ---
combined_data <- data_time %>%
  bind_cols(data_intensity %>% select(starts_with("Intensities")))
...
file_info <- data.frame(
  file = c(
    file.path(mzml_folder, "240925-Exp-PAN-1-010-NoWash-NIST.mzML"),
    file.path(mzml_folder, "240925-Exp-PAN-1-016-NoWash-NIST-R120.mzML"),
    file.path(mzml_folder, "240925-Exp-PAN-1-028-NoWash-NIST-R240.mzML")
  ),
...
# Save
ggsave(
  filename = file.path(output_dir, "Figure_SI-X Article.png"),
  plot = spectrum_glucose,
  width = 20, height = 20, units = "cm", dpi = 1000
)

ggsave(
  filename = file.path(output_dir, "Figure_SI-X Article.tiff"),
  plot = spectrum_glucose,
  width = 20, height = 20, units = "cm",
  dpi = 1000, device = "tiff", compression = "lzw"
)
...
### Using the training set and plot the different correction type

```{r}

# =============================
# 1. Training RSDs for ALL combinations
# =============================

rsd_train_all <- train_df %>%
  group_by(Feature, PCI, CorrectionType) %>%
  summarise(
    train_rsd = 100 * sd(CorrectedArea, na.rm = TRUE) /
      mean(CorrectedArea, na.rm = TRUE),
    .groups = "drop"
  )

# =============================
# 2. Validation RSDs for ALL combinations
# =============================

rsd_val_all <- val_df %>%
  group_by(Feature, PCI, CorrectionType) %>%
  summarise(
    RSD_val = 100 * sd(CorrectedArea, na.rm = TRUE) /
      mean(CorrectedArea, na.rm = TRUE),
    .groups = "drop"
  )

# =============================
# 3. Uncorrected validation RSD
# =============================
...
# Calculate uncorrected RSD per feature
# then duplicate across PCI values
uncorr_val <- uncorrected_df %>%
  filter(Set == "Validation") %>%
  group_by(Feature) %>%
  summarise(
    RSD_uncorrected = 100 * sd(CorrectedArea, na.rm = TRUE) /
      mean(CorrectedArea, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  tidyr::crossing(all_pci)

# =============================
# 4. Merge corrected + uncorrected
# =============================

validation_all <- rsd_val_all %>%
  left_join(
    uncorr_val,
    by = c("Feature", "PCI")
  ) %>%
  mutate(
    Improvement = RSD_uncorrected - RSD_val,
    Percent_Improvement = 100 * Improvement / RSD_uncorrected
  )

# =============================
# 5. Add uncorrected as its own group
# =============================
...
stat_test <- compare_means(
  RSD_val ~ CorrectionType,
  data = plot_df,
  group.by = "PCI",
  ref.group = "uncorrected",
  method = "wilcox.test"
)

# Create non-overlapping y positions
stat_test <- stat_test %>%
  left_join(
    plot_df %>%
      group_by(PCI) %>%
      summarise(
        ymax = max(RSD_val, na.rm = TRUE),
        .groups = "drop"
      ),
    by = "PCI"
  ) %>%
  ...
  arrange(group2) %>%
  mutate(
    y.position = ymax * (1 + 0.08 * row_number())
  ) %>%
  ungroup()

# Optional: inspect p-values
print(
  stat_test %>%
    select(PCI, group1, group2, p, p.adj, p.signif)
)
...
p1 <- ggplot(
  ...
  labs(
    title = "Validation RSD across correction strategies",
    x = "Correction Type",
    y = "Validation RSD (%)"
  ) +
...
# ---------------------------
# 1. Training RSDs for corrected data
# ---------------------------
...
# validation RSD shown,
# selection based on training
# ---------------------------
...
stat_test <- plot_df %>%
  ...
  mutate(
    ...
    y.position = max(plot_df$RSD_val, na.rm = TRUE) *
      (1 + 0.08 * row_number())
  )
...
p1 <- ggplot(
  ...
  labs(
    x = "",
    y = "Validation RSD (%)",
    title = "Best correction method vs uncorrected",
    subtitle = "Selection based on training RSD; plotted values are validation RSD"
  )

print(p1)
```

# SI-Overview improved vs worsened.
...
max_rt <- 15 #ceiling(max(rsd_delta_rt_combined$RT_min, na.rm = TRUE))
...
ggsave(
  filename = file.path(output_dir, "Figure_SI_RSD_change_RT_POS_NEG.png"),
  plot = rt_delta_plot,
  width = 20,
  height = 25,
  units = "cm",
...
ggsave(
  filename = file.path(output_dir, "Figure_SI_RSD_change_RT_POS_NEG.tiff"),
  plot = rt_delta_plot,
  width = 20,
  height = 25,
  units = "cm",
...
## Improvement changes and mean

```{r}
RSD_CUTOFF <- 5

# ============================================================
# 1. Prepare arrow plot data
# ============================================================

rsd_rt_arrow <- plot_df %>%
  filter(Type != "Uncorrected") %>%
  mutate(
    Uncorrected = RSD_uncorrected,
    Corrected   = RSD_val,
  ...
  filter(Change_Class != "Less affected") %>%
  left_join(
    result_long %>%
      group_by(Feature) %>%
      summarise(
...
# ============================================================
# 3. Summary table per correction type
# ============================================================

performance_summary <- rsd_rt_arrow %>%
  group_by(Type) %>%
  summarise(
    Total_n = n(),
    
    n_improved = sum(Change_Class == "Improved"),
    n_worsened = sum(Change_Class == "Worsened"),
    
    percent_improved =
      100 * n_improved / Total_n,
    
    percent_worsened =
      100 * n_worsened / Total_n,
    
    improve_to_worsen_ratio =
      n_improved / pmax(n_worsened, 1),
    
    median_delta_RSD =
      median(-Delta_RSD, na.rm = TRUE),
    
    mean_delta_RSD =
      mean(-Delta_RSD, na.rm = TRUE),
...
aligned_theme <- theme_pubr(base_size = 13) +
  theme(
...
# ============================================================
# 5. Mean lines per Type and Change class
# ============================================================

mean_lines <- rsd_rt_arrow %>%
  group_by(Type, Change_Class) %>%
  summarise(
    Mean_Uncorrected = mean(Uncorrected, na.rm = TRUE),
    Mean_Corrected   = mean(Corrected, na.rm = TRUE),
    n = n(),
...
arrow_plot <- ggplot(rsd_rt_arrow) +
  ...
  geom_text(
    ...
    aes(
      x = Inf,
      y = pmax(Mean_Uncorrected, Mean_Corrected),
      label = paste0(
        "n = ",
        n
      )
  ...
  labs(
    ...
    y = "Validation RSD (%)",
    color = "Outcome",
    title = "Validation RSD magnitude changes across retention time",
    subtitle = paste0(
      "Only features with |ΔRSD| ≥ ",
      RSD_CUTOFF,
      "% shown"
    )
  ) +
...
### How many improved from above 20 to under 20

```{r}
RSD_THRESHOLD <- 20

threshold_crossing_table <- rsd_rt_arrow %>%
  group_by(Type) %>%
  summarise(
    Total_n = n(),

    n_improved = sum(Delta_RSD < 0, na.rm = TRUE),
    n_worsened = sum(Delta_RSD > 0, na.rm = TRUE),

    above20_to_under20 = sum(
      Uncorrected > RSD_THRESHOLD &
        Corrected <= RSD_THRESHOLD,
      na.rm = TRUE
    ),

    under20_to_above20 = sum(
      Uncorrected <= RSD_THRESHOLD &
        Corrected > RSD_THRESHOLD,
      na.rm = TRUE
    ),
    ...
    percent_above20_to_under20_of_improved =
      100 * above20_to_under20 / n_improved,

    percent_under20_to_above20_of_worsened =
      100 * under20_to_above20 / n_worsened,
...
print(threshold_crossing_table)
```

Tryptophan retest performance\
\
...
```{r}
# ---------------------------
# 1. Reshape data (pool all corrections)
# ---------------------------
box_plot_summary <- validation_summary %>%
  pivot_longer(
    cols = c(Mean_RSD_val_best, Mean_RSD_val_uncorr),
    names_to = "Type",
    values_to = "RSD"
  ) %>%
  mutate(Type = recode(Type,
    "Mean_RSD_val_best" = "Corrected",
    "Mean_RSD_val_uncorr" = "Uncorrected"
  ))

# ---------------------------
# 2. Median values
# ---------------------------
medians <- box_plot_summary %>%
  group_by(Type) %>%
  summarise(med = median(RSD, na.rm = TRUE), .groups = "drop")

# ---------------------------
# 3. Paired Wilcoxon test
# ---------------------------
paired_df <- validation_summary %>%
  select(Feature, Mean_RSD_val_best, Mean_RSD_val_uncorr) %>%
  drop_na()

stat <- wilcox.test(
  paired_df$Mean_RSD_val_best,
  paired_df$Mean_RSD_val_uncorr,
  paired = TRUE
)
...
# 4. Plot (VIOLIN)
# ---------------------------
ggplot(box_plot_summary, aes(x = Type, y = RSD, fill = Type)) +
  
  geom_violin(trim = FALSE, alpha = 0.6) +
  
  # optional: add boxplot inside violin
  geom_boxplot(width = 0.15, outlier.shape = NA, alpha = 0.7) +
  
  # median labels
  geom_text(
    data = medians,
    aes(x = Type, y = med, label = round(med, 1)),
    vjust = -0.5,
    inherit.aes = FALSE,
    size = 4
  ) +
  ...
  annotate(
    "text",
    x = 1.5,
    y = max(box_plot_summary$RSD, na.rm = TRUE) * 1.05,
  ...
  labs(
    x = "",
    y = "Mean RSD",
    title = "Overall Comparison: Corrected vs Uncorrected"
  )
```

### Best Correction type boxplot
...
# ---------------------------
box_plot_summary <- validation_summary %>%
  filter(Best_Correction != "uncorrected") %>%
  pivot_longer(
    cols = c(Mean_RSD_val_best, Mean_RSD_val_uncorr),
    names_to = "Type",
    values_to = "RSD"
...
# 3. Y positions
# ---------------------------
y_pos <- box_plot_summary %>%
  group_by(Best_Correction) %>%
  summarise(y = max(RSD, na.rm = TRUE), .groups = "drop")

counts <- left_join(counts, y_pos, by = "Best_Correction")

# ---------------------------
# 4. Medians
# ---------------------------
medians <- box_plot_summary %>%
  group_by(Best_Correction, Type) %>%
  summarise(med = median(RSD, na.rm = TRUE), .groups = "drop")

# ---------------------------
# 5. Statistical testing
# ---------------------------
stat_results <- validation_summary %>%
  filter(Best_Correction != "uncorrected") %>%
  group_by(Best_Correction) %>%
  summarise(
    p_value = {
      diff <- Mean_RSD_val_best - Mean_RSD_val_uncorr
      
      if (shapiro.test(diff)$p.value > 0.05) {
        t.test(Mean_RSD_val_best, Mean_RSD_val_uncorr, paired = TRUE)$p.value
      } else {
        wilcox.test(Mean_RSD_val_best, Mean_RSD_val_uncorr, paired = TRUE)$p.value
      }
    },
    .groups = "drop"
  ) %>%
  ...
  left_join(y_pos, by = "Best_Correction")

# ---------------------------
# 6. Plot (VIOLIN)
# ---------------------------
ggplot(box_plot_summary, aes(x = Best_Correction, y = RSD, fill = Type)) +
  ...
  labs(
    ...
    y = "Mean RSD",
    fill = "Type",
    title = "Corrected vs Uncorrected RSD by Correction Method"
  )

```

### Calculating Bias in validation set

```{r}
#Using uncorrected_df to calculate the expected true value.

mean_validation_area <- uncorrected_df %>%
  filter(Set == "Validation") %>%
  group_by(Feature, CorrectionType) %>%
  summarise(
    Mean_Area = mean(CorrectedArea, na.rm = TRUE),
    .groups = "drop"
  )

mean_validation_area
```
...
table.test <- result_long %>%
select( Wash, mean_rt, Replicate, Feature, Compound, Correction_Type, Corrected_Value) %>%
dplyr::rename(
Treatment = Wash,
Replicate = Replicate,
Feature = Feature,
...
val_df <- split_df %>%
  ...
  group_by(Feature) %>%
  mutate(
    Mean_uncorrected_val = mean(
      CorrectedArea, #[CorrectionType == "uncorrected"],
      na.rm = TRUE
    )
  ) %>%
  ungroup()
...
rsd_train <- train_df %>%
  ...
  select(Feature, PCI, CorrectionType, train_rsd, mean_rt) %>%
...
val_best <- val_df %>%
  ...
  mutate(
    # Absolute bias (difference from reference)
    Bias = CorrectedArea - Mean_uncorrected_val,
    
    # Percent bias (more interpretable)
    Percent_Bias = 100 * Bias / Mean_uncorrected_val
  ) %>%
  group_by(Feature, PCI, CorrectionType) %>%
  mutate(
    Mean_RSD_val_best = 100 * sd(CorrectedArea, na.rm = TRUE) /
                           mean(CorrectedArea, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  dplyr::rename(
    Best_PCI = PCI,
    Best_Correction = CorrectionType
  )
...
uncorr_val <- %>%
  filter(Set == "Validation") %>%
  semi_join(val_best, by = "Feature") %>%
  
  # keep raw values
  mutate(Uncorrected_value = CorrectedArea) %>%
  
  group_by(Feature, PCI) %>%
  summarise(
    RSD_val_uncorr = 100 * sd(CorrectedArea, na.rm = TRUE) /
                           mean(CorrectedArea, na.rm = TRUE),
    
    Mean_RSD_val_uncorr = mean(RSD_val_uncorr, na.rm = TRUE),
    
    # keep all raw values as list-column (so nothing is lost)
    Uncorrected_value = list(Uncorrected_value),
...
validation_summary <- val_best %>%
  ...
  ungroup() %>%
  mutate(
    Bias_uncorrected = Uncorrected_value - Mean_uncorrected_val,
    Percent_Bias_uncorrected = 100 * Bias_uncorrected / Mean_uncorrected_val
  ) %>%
  mutate(
    Delta_Bias = Percent_Bias_uncorrected - Percent_Bias
  )

bias_long <- validation_summary %>%
  select(mean_rt, Percent_Bias, Percent_Bias_uncorrected,Treatment) %>%
  pivot_longer(
    cols = c(Percent_Bias, Percent_Bias_uncorrected),
    names_to = "Type",
    values_to = "Bias"
  ) %>%
  mutate(
    Type = recode(Type,
      "Percent_Bias" = "Corrected",
      "Percent_Bias_uncorrected" = "Uncorrected"
    )
  )

ggplot(bias_long, aes(x = mean_rt, y = Bias, color = Type)) +
  ...
  labs(
    ...
    y = "Percent Bias (%)",
    color = "",
    title = "Corrected vs Uncorrected Bias Across Chromatogram"
  ) +
...
### Testing set heatmap plot

```{r}

# 1. Prepare matrix
heatmap_data <- best_combo %>%
  select(Feature, train_rsd, mean_rt) %>%
...
pheatmap(
  ...
  main = "Feature Clustering (RSD + RT)"
)
```
...
heatmap_data <- validation_summary %>%
  filter( Best_Correction != "uncorrected") %>%
  select(Feature, Treatment, Percent_Improvement, mean_rt) %>%
  
  # reshape to Feature × Treatment matrix
  pivot_wider(
    names_from = Treatment,
    values_from = Percent_Improvement
  ) %>%
  
  # order by RT (important for interpretation)
  arrange(mean_rt)
...
# =============================
# 3. Create RT annotation
# =============================

annotation <- data.frame(
  RT = heatmap_data$mean_rt
)
...
pheatmap(
  mat,
  scale = "row",  # 🔥 focus on patterns across treatments
  annotation_row = annotation,
  clustering_distance_rows = "euclidean",
  clustering_method = "complete",
  main = "Clustering of Features by RSD Improvement"
)

# =============================
# 5. Extract clusters (optional but recommended)
# =============================

hc <- hclust(dist(mat))
clusters <- cutree(hc, k = 3)   # choose number of clusters

annotation$Cluster <- factor(clusters)

# =============================
# 6. Heatmap with cluster annotation
# =============================
...
pheatmap(
  ...
  clustering_distance_rows = "euclidean",
  clustering_method = "complete",
  main = "Feature Clustering (RSD Improvement + RT)"
)

# =============================
# 7. Check RT dependence (key analysis)
# =============================

rt_check <- validation_summary %>%
  group_by(Feature) %>%
  summarise(
    mean_RT = mean(mean_rt, na.rm = TRUE),
...
ggplot(rt_check, aes(x = mean_RT, y = mean_improvement)) +
  geom_point(alpha = 0.6) +
  geom_smooth(se = FALSE) +
  labs(
    x = "Retention Time (min)",
    y = "Mean RSD Improvement (%)",
    title = "RT Dependence of RSD Improvement"
  ) +
  theme_minimal()
```

### Extract NIST amino acid
...
mzml_folder <- "C:/Users/DanielMalheiro/Downloads/Exp7-241008/mzML-PAN"
file.exists(file.path(mzml_folder, files))

# Define function to extract EIC for a target m/z
extract_eic <- function(file_name, target_mz, ppm, mzml_folder) {
  
  file_path <- file.path(mzml_folder, file_name)
  raw_data <- xcmsRaw(file_path)
  
  # correct polarity handling
  positive_scans <- which(raw_data@polarity == 1)
  
  if (length(positive_scans) == 0) return(NULL)
  ...
  eic_df <- data.frame(
    ...
    mutate(
      average_intensity = zoo::rollmean(intensity, k = 3, fill = NA, align = "left")
    )
...
ppm <- 2
#all masses are M+H ions
target_mz <- c(
              205.0972,   # Tryptophan m/z value
              90.05495,  # Alanine
              76.0393,  #Glycine
              132.1019,  #Leucine/Isoleucine
              147.1128,  #Lysine
              150.05833,  # Methionine
              116.0706, #Proline
              106.04987, #Serine
              182.08117, #Tyrosine
            118.08625 #Valine
...
eic_list_certifed <- lapply(files, function(file_name) {
  bind_rows(
    lapply(target_mz, function(mz) {
      extract_eic(file_name, mz, ppm, mzml_folder)
    })
  )
})


# saveRDS(eic_list_cert, file = "eic_list_cert.rds")
```
...
# FAST extractor
# =============================
extract_eic_fast <- function(file_name, target_mz, ppm, mzml_folder) {

  file_path <- file.path(mzml_folder, file_name)
  raw_data <- xcmsRaw(file_path)

  scans <- which(raw_data@polarity == "positive")

  if (length(scans) == 0) {
    message("No positive scans in: ", file_name)
    return(NULL)
  }

  eic_list <- lapply(target_mz, function(mz) {

    mz_tol <- (ppm / 1e6) * mz
    mz_range <- c(mz - mz_tol, mz + mz_tol)

    eic <- rawEIC(raw_data, mzrange = mz_range)

    data.frame(
      rt = raw_data@scantime[scans] / 60,
      intensity = eic$intensity[scans],
      target_mz = mz,
      file_name = file_name
    )
  })
  ...
  bind_rows(eic_list) %>%
    group_by(file_name, target_mz) %>%
    mutate(
      avg_intensity = zoo::rollmean(intensity, k = 3, fill = NA, align = "left")
    ) %>%
    ungroup()
}
...
# Run ALL files
# =============================
eic_fast_OG <- lapply(files, function(file_name) {
  extract_eic_fast(file_name, target_mz, ppm, mzml_folder)
}) %>%
  bind_rows()

mz_lookup <- tibble::tibble(
  target_mz = c(
    205.0972,
    90.05495,
    76.0393,
...
eic_fast <- eic_fast_OG %>%
  left_join(mz_lookup, by = "target_mz") %>%
  mutate(
    target_label = ifelse(
      is.na(Target_compound),
      paste0("m/z ", round(target_mz, 4)),
      paste0(Target_compound, " (m/z ", round(target_mz, 4), ")")
    )
  )

# Combine all EICs into a single data frame
#combined_eic_df2 <- bind_rows(eic_fast)

# Filter data to the retention time between 0 and 6 minutes
combined_eic_df_filtered2 <- eic_fast %>%
  filter(rt >= 0 & rt <= 6) %>%
    separate(
    file_name,
    into = c(
...
# View the result
print(combined_eic_df_filtered2)


combined_eic_df_filtered2 %>%
  ggplot(aes(x = rt, y = intensity, color = Wash)) +
  geom_line(linewidth = 1) +
  facet_wrap( ~Wash ~Replicate , ncol = 3) 
  labs(
...
# Calculate Matrix Effect (ME) and Matrix Effect Factor (MEF) for each NIST replicate
matrix_effect_data <- filtered_data_3 %>%
  filter(Sample %in% c("NIST", "Solvent")) %>%  
  # First, calculate the average Solvent for each Scan and Wash (grouping by Scan and Wash)
  group_by(Sequence, Wash, Scan, Compound, Sample, Replicate) %>%
  summarise(
...
# =============================
ggplot(
  eic_fast_named %>% filter(between(rt, 0, 6                        )),
  aes(x = rt, y = avg_intensity, color = file_name)
) +
  geom_line(
    aes(group = interaction(file_name, target_mz)),
    alpha = 0.6
  ) +
  facet_wrap(~ Target_compound.x, scales = "free_y") +
  labs(
    x = "Retention Time (min)",
    y = "Intensity",
    title = "EICs (All Files, 0–6 min)"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
```

### Add ME Plot 1 compound trace
...
ggplot(
  eic_fast_df %>% 
    filter(target_mz == 90.05495),
  aes(x = rt, y = avg_intensity, color = factor(Replicate))
) +
  geom_line(alpha = 0.8) +
  
  facet_grid(~ Wash ~ Replicate, scales = "free_y") +
  
  labs(
    x = "Retention Time (min)",
    y = "Intensity",
    color = "Replicate",
    title = "Alanine (m/z 90.05495) Across Wash Conditions"
  ) +
...
```{r}
# =============================
# 1. Find peak apex per group
# =============================
peak_rt_df <- peak_rt_df %>%
  mutate(
    closest_time = purrr::map_dbl(
      rt,
      ~ matrix_effect_data$Times[which.min(abs(matrix_effect_data$Times - .))]
    ),
    closest_scan = purrr::map_dbl(
      closest_time,
      ~ matrix_effect_data$Scan[which.min(abs(matrix_effect_data$Times - .))]
    )
  ) %>%
  mutate(
    scan = as.numeric(scan),
    closest_scan = as.numeric(closest_scan),
    scan_diff = scan - closest_scan
  )

# =============================
# 2. Match closest MEF scan/time
# =============================
peak_rt_df <- peak_rt_df %>%
  mutate(
    closest_time = purrr::map_dbl(
...
# 3. Apply scan shift
# =============================
eic_shifted <- eic_fast_df %>%
  left_join(
    peak_rt_df %>% select(Wash, Replicate, target_mz, scan_diff),
    by = c("Wash", "Replicate", "target_mz")
  ) %>%
  mutate(
    adjusted_scan = scan - scan_diff
  )

# =============================
# 4. Join Matrix Effect Factor
# =============================
eic_shifted <- eic_shifted %>%
  left_join(
    matrix_effect_data %>%
      select(Wash, Replicate, Scan, Matrix_Effect_Factor, Compound),
    by = c("Wash", "Replicate", "adjusted_scan" = "Scan")
  ...
  mutate(
    corrected_intensity = avg_intensity / Matrix_Effect_Factor
  )

# =============================
# 5. Pivot + summarise
# =============================
eic_long <- eic_shifted %>%
  pivot_longer(
    cols = c(avg_intensity, corrected_intensity),
    names_to = "Intensity_Type",
    values_to = "Intensity_Value"
  ) %>%
  group_by(Wash, rt, Replicate, Intensity_Type, target_mz) %>%
  summarise(
    Average_Intensity = mean(Intensity_Value, na.rm = TRUE),
    Matrix_Effect_Factor = mean(Matrix_Effect_Factor, na.rm = TRUE),
    .groups = "drop"
  )
...
# =============================
scaling_factor <-
  max(eic_long$Average_Intensity, na.rm = TRUE) /
  max(eic_long$Matrix_Effect_Factor, na.rm = TRUE)

# =============================
# 7. Plot (Alanine example)
# =============================
alanine_mz <- 90.05495
...
ggplot(
  ...
  # Intensity traces
  geom_line(
    aes(y = Average_Intensity, color = Intensity_Type),
    linewidth = 0.6
  ) +
  
  # Matrix effect factor (secondary axis)
  geom_line(
    aes(
      y = Matrix_Effect_Factor * scaling_factor,
      color = "Matrix Factor"
  ...
  labs(
    x = "Retention Time (min)",
    y = "Intensity",
    title = "Alanine (m/z 90.05495) with Matrix Effect Correction"
  ) +
...
```{r}
# =============================
# 1. Combine EICs + add Compound + scan
# =============================
combined_eic_df <- bind_rows(eic_fast)

# TEMP fix: manually assign Alanine (since you're only testing that)
combined_eic_df <- combined_eic_df %>%
  group_by(file_name, target_mz) %>%
  mutate(scan = row_number()) %>%
  ungroup()

# =============================
# 2. Filter + annotate metadata
# =============================
combined_eic_df_filtered <- combined_eic_df %>%
  filter(
    rt >= 0 & rt <= 6,
    Target_compound == "Alanine"   # 🔥 focus on Alanine
  ) %>%
  separate(
    file_name,
    into = c("Date","Exp","PAN","Batch","Sequence","Wash","Sample"),
    sep = "-", remove = FALSE
  ) %>%
  mutate(
    Sample = str_remove(Sample, "\\.mzML"),
...
# =============================
# 3. Compute MEF (FIXED DATASET NAME)
# =============================
filtered_data_3 <- long_data %>%
  filter(
    between(Times, 0, 6),
    Compound %in% c("Glucose-13C6","Leucine-d10","Tryptophan-d5"),
    Sample %in% c("NIST","Solvent")
  )

matrix_effect_data <- filtered_data_3 %>%
  group_by(Sequence, Wash, Scan, Compound, Sample, Replicate) %>%
  summarise(
    value = mean(Intensities, na.rm = TRUE),
    Times = dplyr::first(Times),
    .groups = "drop"
  ) %>%
  pivot_wider(names_from = Sample, values_from = value) %>%
  group_by(Wash, Scan, Compound) %>%
  mutate(Solvent_mean = mean(Solvent, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(
    Matrix_Effect_Factor = NIST / Solvent_mean
  )

# =============================
# 4. Find peak apex
# =============================
peak_rt_df <- combined_eic_df_filtered %>%
  group_by(Wash, Replicate) %>%
  filter(rt == rt[which.max(intensity)]) %>%
  ungroup()
...
peak_rt_df <- peak_rt_df %>%
  rowwise() %>%
  mutate(
    closest_scan = matrix_effect_data %>%
      filter(
        Wash == cur_data()$Wash,
        Replicate == cur_data()$Replicate
      ) %>%
      slice_min(abs(Times - rt), n = 1, with_ties = FALSE) %>%  # 🔥 important
      pull(Scan),
...
# =============================
# 6. Apply scan shift + join MEF
# =============================
combined_eic_df_with_shift <- combined_eic_df_filtered %>%
  left_join(
    peak_rt_df %>% select(Wash, Replicate, scan_diff),
    by = c("Wash","Replicate")
  ) %>%
  mutate(
    adjusted_scan = scan - scan_diff
  ...
  left_join(
    matrix_effect_data %>%
      select(Wash, Replicate, Scan, Matrix_Effect_Factor, Compound),
    by = c("Wash","Replicate","adjusted_scan" = "Scan"),
    relationship = "many-to-many"   # 🔥 important
  ) %>%
  mutate(
    corrected_intensity = ifelse(
      is.na(Matrix_Effect_Factor),
      NA,
      intensity / Matrix_Effect_Factor
    )
  )
...
# 7. Pivot + summarise
# =============================
combined_eic_df_long <- combined_eic_df_with_shift %>%
  pivot_longer(
    cols = c(intensity, corrected_intensity),
    names_to = "Intensity_Type",
    values_to = "Intensity_Value"
  ) %>%
  group_by(Wash, rt, Replicate, Intensity_Type, Compound) %>%  # 🔥 keep PCI
  summarise(
    Average_Intensity = mean(Intensity_Value, na.rm = TRUE),
    Matrix_Effect_Factor = mean(Matrix_Effect_Factor, na.rm = TRUE),
    .groups = "drop"
...
# =============================
scaling_factor <-
  max(combined_eic_df_long$Average_Intensity, na.rm = TRUE) /
  max(combined_eic_df_long$Matrix_Effect_Factor, na.rm = TRUE)

# =============================
# 9. Final plot
# =============================
ggplot(
  combined_eic_df_long %>% filter(between(rt, 0.5, 1.5),
    Wash == "22x"),
  aes(x = rt)
) +
  geom_line(
  ...
  geom_hline(
    yintercept = 1 * scaling_factor,
    linetype = "dashed",
    color = "red"
  ) +
  facet_grid(Compound ~ Wash + Replicate) +
  scale_y_continuous(
    name = "Intensity",
    sec.axis = sec_axis(~ . / scaling_factor, name = "Matrix Factor")
  ) +
  scale_color_manual(
    values = c(
      "Uncorrected" = "blue",
...
# =============================
target_to_plot <- "Tryptophan" #"Tryptophan",
#"Alanine",
#"Glycine",
#"Leucine/Isoleucine", rt 2.5 - 3.5
#"Lysine",
#"Methionine",
#"Proline",
#"Serine",
...
mz_lookup <- tibble::tibble(
  target_mz = c(
    205.0972, 90.05495, 76.0393, 132.1019, 147.1128,
    150.05833, 116.0706, 106.04987, 182.08117, 118.08625
  ),
  Target_compound = c(
    "Tryptophan", "Alanine", "Glycine", "Leucine/Isoleucine", "Lysine",
    "Methionine", "Proline", "Serine", "Tyrosine", "Valine"
  )
)
...
combined_eic_df_filtered <- combined_eic_df %>%
  filter(
    between(rt, 0, 6),
    Target_compound == target_to_plot
  ) %>%
  separate(
    file_name,
    into = c("Date","Exp","PAN","Batch","Sequence","Wash","Sample"),
...
peak_rt_df <- peak_rt_df %>%
  ...
  mutate(
    closest_scan = matrix_effect_data %>%
      ...
      slice_min(abs(Times - rt), n = 1, with_ties = FALSE) %>%
      pull(Scan),
    scan_diff = scan - closest_scan
  ) %>%
  ungroup()
...
combined_eic_df_long <- combined_eic_df_with_shift %>%
  ...
  group_by(Wash, rt, Replicate, Intensity_Type, Compound) %>%
  summarise(
    Average_Intensity = mean(Intensity_Value, na.rm = TRUE),
    Matrix_Effect_Factor = mean(Matrix_Effect_Factor, na.rm = TRUE),
    .groups = "drop"
...
# =============================
ggplot(
  combined_eic_df_long %>%
    filter(
      between(rt, 4.75, 5.25),
      #Wash == "04x"
    ),
  aes(x = rt)
) +
  ...
  labs(
    x = "Retention Time (min)",
    y = "Intensity",
    title = paste0(target_to_plot, " with Matrix Effect Correction")
  ) +
  theme_minimal() +
...
# Step 0: USER INPUT
# ===============================
target_to_plot <- "Leucine/Isoleucine"
rt_min <- 2.5   # lower RT bound (adjust)
rt_max <- 3.5   # upper RT bound (adjust)

# ===============================
# Step 1: Use correct dataset
# ===============================
data_for_integration <- combined_eic_df_long 
  #filter(Target_compound == target_to_plot)

# ===============================
# Step 2: Trapezoidal integration
# ===============================
integrated_data <- data_for_integration %>% 
  filter(
    !is.na(rt),
    !is.na(Average_Intensity),
    !is.na(Compound),
  ...
  group_by(Wash, Replicate, Intensity_Type, Compound) %>%
  summarise(
    integration_area = trapz(rt, Average_Intensity),
    .groups = "drop"
  )
...
integrated_data_long_wide <- integrated_data_wide %>%
  pivot_longer(
    cols = -c(Wash, Replicate, Compound),
    names_to = "Intensity_Type",
    values_to = "integration_area"
  )

# ===============================
# Step 5: Keep ONE uncorrected + all corrected
# ===============================
integrated_data_long_wide_filtered <- integrated_data_long_wide %>%
  
  group_by(Wash, Replicate) %>%
  filter(!(Intensity_Type == "Uncorrected" & duplicated(Intensity_Type))) %>%
  ungroup() %>%
  
  mutate(
    Compound = if_else(
      Intensity_Type == "Uncorrected",
      "Uncorrected",
      Compound
    )
  ) %>%
  ...
  filter(
    Intensity_Type != "Uncorrected" | Compound == "Uncorrected"
  ) %>%
...
barplot_compounds_with_error <- ggplot(
  error_data,
  aes(x = Compound_plot, y = Mean, fill = Compound_plot)
) +
  geom_bar(
    stat = "identity",
    color = "black",
  ...
  geom_text(
    aes(
      y = Mean + SE + 0.05 * max(Mean, na.rm = TRUE),
      label = paste0(signif(RSD_percent, 2), "%")
    ),
    size = 3.5,
    vjust = -0.5
  ...
  facet_wrap(~Wash, ncol = 4, drop = TRUE) +
  labs(
    x = NULL,
    y = "Integrated Area",
    title = paste0("Integration Comparison: ", target_to_plot)
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "none",
...
### New Homologue plots with MSMS data

```{r}
# Sup_info <- here("Data","Supplementary Data.xlsx")
# data3<- read_excel(Sup_info, sheet = "Feature List")

# New MSMS data to add oto merged_df 

Sup_info <- "C:/Users/DanielMalheiro/Clinical Microbiomics/Teamwebsted - MS-Omics Team Folder/Daniel/PhD/Experiments/Exp7-PCI with Samples/CompoundswithMS2.xlsx"
dataMSMS<- read_excel(Sup_info)
colnames(dataMSMS)

peak_table1 <- dataMSMS %>%
  select(
    "Name",
    "Formula",
    "m/z",
...
summary_table_F <- filtered_data %>%
  ...
  mutate(
    Wash = recode(Wash, "NoWash" = "PLR unwashed"),
...
MS_dilution <- all_combined_spectra %>% 
  filter(Replicate == 1,
         scan %in% c("Mass Spectrum 2","Mass Spectrum 4")) %>%
  ggplot(aes(x = mz, y = intensity)) +
  geom_line(linewidth = 1, aes(color = scan)) +
  geom_vline(xintercept = target_masses, linetype = "dashed", color = "black") +
  facet_grid(~Dilution ~ scan, scales = "free_x") +
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure_12_2.png"),
  plot = MS_dilution,
  width = 10,
  height = 10,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure_12_2.tiff"),
  plot = MS_dilution,
  width = 10,
  height = 10,
  units = "cm",
...
# Read MS2 data
MS2_data <- read_excel(
  "C:/Users/DanielMalheiro/Clinical Microbiomics/Teamwebsted - MS-Omics Team Folder/Daniel/PhD/Experiments/Exp7-PCI with Samples/CompoundswithMS2.xlsx",
  col_types = "text"
) %>%
  select(1:26) %>%
  rename(
    mz = `m/z`,
    rt = `RT [min]`,
    Adduct = `Reference Ion`
  ) %>%
  mutate(
    across(c(mz, rt, `Calc. MW`), as.numeric)
  )

# Check duplicate MS2 matches
duplicates_MS2 <- MS2_data %>%
  count(mz, rt, Adduct) %>%
  filter(n > 1)

if (nrow(duplicates_MS2) > 0) {
  print(duplicates_MS2)
  stop("Multiple matches found in MS2_data for mz, rt, and Adduct. Inspect duplicates before joining.")
}
  ...
  left_join(
    MS2_data %>%
      select(
        ...
        MS2,
        SMILES_new = SMILES,
        InChI_Key_new = `InChI Key`
      ),
    by = c("mz", "rt", "Adduct"),
    relationship = "many-to-one"
  ) %>%
  mutate(
    Name = coalesce(Name_new, Name),
    SMILES = coalesce(SMILES_new, SMILES),
    `InChI Key` = coalesce(InChI_Key_new, `InChI Key`),
    
`Annotation level` = case_when(
  MS2 %in% c("DDA for preferred ion", "DDA for other ion") & !is.na(Name) ~ 2,
  !is.na(Name) ~ 3,
  is.na(Name) ~ 4
)
  ) %>%
  select(-Name_new, -SMILES_new, -InChI_Key_new)


# Path to cached classification file
largelist_path2 <- here("Data", "largelist_MS2_update.rds")

if (file.exists(largelist_path2)) {
  
  classification_list2 <- readRDS(largelist_path2)
  
} else {
  
  # Extract unique InChI Keys
  InChI_Keys2 <- data3update %>%
    pull(`InChI Key`) %>%
    na.omit() %>%
    unique() %>%
    as.list()
  ...
  safe_get_classification <- possibly(
    get_classification,
    otherwise = NULL
  )
  
  # Run classification with progress bar
  classification_list2 <- pblapply(
    InChI_Keys2,
    safe_get_classification
  )
  
  # Save results for future runs
  saveRDS(classification_list2, largelist_path)
}

# Optional inspection
meta(classification_list2[[1]])
classification(classification_list2[[1]])

# Function to safely extract classification data
extract_classification_data <- function(class_obj) {
  if (is.null(class_obj)) {
    return(tibble(
      inchikey = NA_character_,
...
parsed_data2 <- map_dfr(
  classification_list2,
  possibly(extract_classification_data, otherwise = tibble())
)

print(parsed_data2)

#Clean InChIKey and keep .y naming for now
...
## Extraction plot for different compound.
...
ppm <- 2
rt_window <- 0.3   # +/- 1 min around expected RT
...
# Check files
print(file.exists(file.path(mzml_folder, files)))

if (any(!file.exists(file.path(mzml_folder, files)))) {
  stop("Some mzML files were not found. Check mzml_folder or file names.")
}

# ============================================================
# 2. Target metabolite list
# ============================================================

targets <- tibble::tribble(
  ~Metabolite,              ~target_mz,  ~rt_expected,
  "L-Lysine",               147.11279,   0.748,
  "L-Histidine",            156.07673,   0.798,
  "Acetylcholine",          146.11756,   0.991,
  "Creatinine",             114.06611,   1.052,
  "Uridine monophosphate",  325.04317,   1.081,
  "Uridine monophosphate",  325.04328,   1.479,
  "Acetyl-L-carnitine",     204.12288,   1.802,
  "Acetyl-L-carnitine",     204.12293,   2.250,
  "L-Tyrosine",             182.08114,   3.492,
  "Paracetamol",            152.07060,   4.012,
  "Paracetamol",            152.07062,   4.677,
  "D-Pantothenic acid",     220.11775,   4.592,
  "DL-Tryptophan",          205.09709,   4.857
)

# ============================================================
# 3. Fast extraction: load each mzML once, extract all targets
# ============================================================

extract_all_targets_from_file <- function(file_name, targets, ppm, rt_window,
                                          polarity_label = "positive") {
  
  message("Reading: ", file_name)
  
  raw_data <- xcmsRaw(file.path(mzml_folder, file_name))
  
  positive_scans <- which(raw_data@polarity == polarity_label)
  rt_all <- raw_data@scantime[positive_scans] / 60
  
  purrr::pmap_dfr(
    targets,
    function(Metabolite, target_mz, rt_expected) {
      ...
      tibble(
        ...
        target = paste0(Metabolite, " m/z ", round(target_mz, 4))
      ) %>%
        filter(
          between(rt, rt_expected - rt_window, rt_expected + rt_window)
        ) %>%
        mutate(
          average_intensity = rollmean(
            intensity,
            k = 3,
            fill = NA,
            align = "left"
...
combined_eic_df_filtered <- purrr::map_dfr(
  files,
  extract_all_targets_from_file,
  targets = targets,
  ppm = ppm,
  rt_window = rt_window
)
...
# ============================================================
# 4. Parse filenames and assign replicates
# ============================================================

combined_eic_df_filtered <- combined_eic_df_filtered %>%
  separate(
    file_name,
    into = c("Date", "Exp", "PAN", "Batch", "Sequence", "Wash", "Sample"),
    sep = "-",
  ...
  group_by(Wash, Sample, Metabolite, target_mz, rt_expected) %>%
  mutate(
...
saveRDS(
  combined_eic_df_filtered,
  file = "combined_eic_df_filtered_all_metabolites.rds"
)

# ============================================================
# 5. Export raw EIC plots to PDF
# ============================================================

pdf_file <- paste0(
  "C:/Users/DanielMalheiro/Downloads/All_Metabolite_EICs_",
  format(Sys.time(), "%Y%m%d_%H%M%S"),
  ".pdf"
)

pdf(file = pdf_file, width = 11, height = 8)

for (met in unique(combined_eic_df_filtered$Metabolite)) {
  
  plot_data <- combined_eic_df_filtered %>%
    filter(Metabolite == met)
  
  mz_label <- paste(round(unique(plot_data$target_mz), 4), collapse = ", ")
  rt_label <- paste(round(unique(plot_data$rt_expected), 3), collapse = ", ")
  ...
  p <- plot_data %>%
    ggplot(
      aes(
        ...
        y = intensity,
        color = Wash,
        group = interaction(Wash, Replicate, target_mz)
      )
    ) +
    geom_line(alpha = 0.8, linewidth = 0.5) +
    geom_vline(
      aes(xintercept = rt_expected),
      linetype = "dashed",
      color = "black"
    ) +
    facet_wrap(~Replicate, ncol = 4) +
    labs(
      title = paste0(met, " | m/z: ", mz_label, " | expected RT: ", rt_label),
      x = "Retention Time (min)",
      y = "Intensity",
      color = "Wash"
    ) +
...
```{r}
# ============================================================
# MEF-corrected EIC plots for all target metabolites
# One metabolite per PDF page
# Similar style to your Tryptophan plot
# ============================================================
...
# Output PDF
# -----------------------------
pdf_file_corrected <- paste0(
  "C:/Users/DanielMalheiro/Downloads/All_Metabolite_EIC_MEF_corrected_",
  format(Sys.time(), "%Y%m%d_%H%M%S"),
  ".pdf"
)

# -----------------------------
# Function to make one metabolite plot
# -----------------------------
plot_one_metabolite_corrected <- function(met_name, eic_df, matrix_effect_data) {
  
  eic_one <- eic_df %>%
    filter(Metabolite == met_name)
  
  # Step 1: find peak RT for each Wash / Replicate / target_mz
  eic_peak <- eic_one %>%
    group_by(Wash, Replicate, target_mz, rt_expected) %>%
    filter(rt == rt[which.max(intensity)]) %>%
    slice(1) %>%
    ungroup()
  
  # Step 2: find closest MEF scan
  eic_peak <- eic_peak %>%
    mutate(
      closest_time = purrr::map_dbl(
        rt,
        ~ matrix_effect_data$Times[which.min(abs(matrix_effect_data$Times - .x))]
      ),
      closest_scan = purrr::map_dbl(
        closest_time,
        ~ matrix_effect_data$Scan[which.min(abs(matrix_effect_data$Times - .x))]
      ),
      scan_diff = scan - closest_scan
    )
  
  # Step 3: apply scan shift
  eic_shifted <- eic_one %>%
    left_join(
      eic_peak %>%
        select(Wash, Replicate, target_mz, rt_expected, scan_diff),
      by = c("Wash", "Replicate", "target_mz", "rt_expected")
    ) %>%
    mutate(
      adjusted_scan = scan - scan_diff
    )
  ...
  eic_shifted <- eic_shifted %>%
    ...
    mutate(
      ...
      Compound_plot = recode(
        ...
        "Leucine-d10"    = "Leucine-d[10]",
        "Glucose-13C6"   = "Glucose-{}^{13}*C[6]",
        "StearicAcid-d35" = "StearicAcid-d[35]",
        "Indole-d6"      = "Indole-d[6]",
        "Carnitine-d9"   = "Carnitine-d[9]"
      )
    )
  ...
  eic_long <- eic_shifted %>%
    ...
    group_by(
      Wash, rt, Matrix_Effect_Factor, Sample, Replicate,
      Intensity_Type, Sequence, Metabolite, target_mz,
      rt_expected, Compound, Compound_plot
    ) %>%
    summarise(
      Average_Intensity = mean(Intensity_Value, na.rm = TRUE),
      .groups = "drop"
    ...
    mutate(
      Compound_plot = factor(Compound_plot, levels = compound_plot_levels),
      Intensity_Type = recode(
        Intensity_Type,
        intensity = paste0("Uncorrected ", met_name, " Intensity"),
        corrected_intensity = paste0("Corrected ", met_name, " Intensity")
      )
    )
  
  # Keep same wash as your example
  plot_data <- eic_long %>%
    filter(Wash == "PLR 4x wash")
  ...
  scaling_factor <-
    max(plot_data$Average_Intensity, na.rm = TRUE) /
    max(plot_data$Matrix_Effect_Factor, na.rm = TRUE)
  
  uncorrected_label <- paste0("Uncorrected ", met_name, " Intensity")
  corrected_label   <- paste0("Corrected ", met_name, " Intensity")
  ...
  plot_data %>%
    ...
    geom_line(
      aes(
        ...
        y = Average_Intensity,
        color = Intensity_Type,
        group = interaction(Intensity_Type, target_mz)
      ),
      linewidth = 0.5
    ) +
    ...
    scale_color_manual(
      values = c(
        uncorrected_label = "blue",
        corrected_label   = "green",
        "Matrix Factor"   = "red"
      )
    ) +
...
# -----------------------------
pdf(pdf_file_corrected, width = 14, height = 9)

for (met in unique(combined_eic_df_filtered$Metabolite)) {
  
  p <- plot_one_metabolite_corrected(
    met_name = met,
    eic_df = combined_eic_df_filtered,
    matrix_effect_data = matrix_effect_data
  )
...
## One compund at a time

```{r}
# ============================================================
# L-Lysine bar plot
# Integrated area: Uncorrected vs corrected by MF Compound
# Similar style to your example
# ============================================================
...
# -----------------------------
# Step 1: Integrate area
# -----------------------------
lysine_integrated_data <- lysine_long %>%
  filter(
    !is.na(rt),
    !is.na(Average_Intensity),
    !is.na(Compound),
...
# -----------------------------
# Step 2: Keep one uncorrected + all corrected compounds
# -----------------------------
lysine_integrated_filtered <- lysine_integrated_data %>%
  group_by(Wash, Replicate) %>%
  filter(!(Intensity_Type == "intensity" & duplicated(Intensity_Type))) %>%
  ungroup() %>%
  mutate(
...
# -----------------------------
lysine_error_data <- lysine_integrated_filtered %>%
  group_by(Wash, Compound) %>%
  summarise(
    Mean = mean(integration_area, na.rm = TRUE),
    SD = sd(integration_area, na.rm = TRUE),
...
# Step 4: Plot labels
# -----------------------------
lysine_error_data <- lysine_error_data %>%
  mutate(
    Compound_plot = recode(
      as.character(Compound),
      "Uncorrected"     = "Uncorrected",
...
# Step 5: Bar plot
# -----------------------------
lysine_barplot_compounds_with_error <- ggplot(
  lysine_error_data,
  aes(
    x = Compound_plot,
    y = Mean,
  ...
  labs(
    title = "L-Lysine integrated area | PLR 4x wash",
    x = NULL,
    y = "Integrated Area"
  ) +
  theme_minimal(base_size = 14) +
...
print(lysine_barplot_compounds_with_error)
```

## trying for 50 metabolites

```{r}
# ============================================================
# 50 unique metabolites, RT 0-10 min
# Extract EICs, correct using MF, integrate, barplot, export PDF
# ============================================================
...
stopifnot(all(file.exists(file.path(mzml_folder, files))))

# -----------------------------
# 50 unique targets, positive mode, RT 0-10 min
# No duplicate metabolite names
# -----------------------------
targets <- tibble::tribble(
  ~Metabolite, ~target_mz, ~rt_expected,
  "L-Lysine", 147.11279, 0.748,
  ...
  "D-Pantothenic acid", 220.11775, 4.592,
  "DL-Tryptophan", 205.09709, 4.857,
  "Betaine", 118.08616, 0.877,
  "Choline", 104.10688, 0.880,
  "Proline", 116.07048, 0.989,
  "L-Valine", 118.08619, 1.299,
  "L-Glutamic acid", 148.06043, 0.842,
  "DL-Arginine", 175.11890, 0.834,
  "Taurine", 126.02189, 0.788,
  "Nicotinamide", 123.05518, 3.919,
  "Caffeine", 195.08766, 5.209,
  "Theobromine", 181.07201, 4.623,
  "Paraxanthine", 181.07198, 4.879,
  "Propionylcarnitine", 218.13864, 3.997,
  "C4-Carnitine", 232.15431, 4.502,
  "Hexanoylcarnitine", 260.18561, 5.540,
  "C8-Carnitine", 288.21693, 6.290,
  "Decanoylcarnitine", 316.24823, 6.789,
  "Adenosine", 268.10390, 4.340,
  "Guanine", 152.05664, 4.162,
  "8-hydroxy-deoxyguanosine", 284.09894, 4.161,
  "N6-Me-Adenosine", 282.11968, 3.861,
  "Cytosine", 112.05045, 2.802,
  "Hypoxanthine", 137.04577, 3.321,
  "Uracil", 113.03448, 3.889,
  "Caprolactam", 114.09125, 5.017,
  "Atenolol", 267.17029, 4.488,
  "Metalaxyl", 280.15434, 5.116,
  "Trazodone", 372.15860, 5.863,
  "Quetiapine", 384.17412, 6.233,
  "Naproxen", 231.10166, 6.800,
  "Piperine", 286.14377, 7.021,
  "Azelaic acid", 189.11212, 6.228,
  "Sebacic acid", 203.12777, 6.101,
  "Dodecanedioic acid", 231.15903, 6.735,
  "Monoolein", 357.29997, 8.143,
  "Oleamide", 282.27913, 8.163,
  "Stearamide", 284.29479, 8.492,
  "LysoPC(P-18:0)", 508.37614, 8.139,
  "Ceramide (d18:1/16:0)", 538.51921, 9.298,
  "C14-Dihydroceramide", 512.50385, 9.155,
  "Calcitriol", 417.33657, 8.811
)

targets <- targets %>%
  filter(
    !duplicated(Metabolite),
    between(rt_expected, 0, 10)
  ) %>%
  slice_head(n = 50)
...
combined_eic_df_filtered_50 <- purrr::map_dfr(
  files,
  extract_all_targets_from_file,
  targets = targets,
  ppm = ppm,
...
combined_eic_df_filtered_50 <- combined_eic_df_filtered_50 %>%
  separate(
    file_name,
    into = c("Date", "Exp", "PAN", "Batch", "Sequence", "Wash", "Sample"),
    sep = "-",
...
saveRDS(
  combined_eic_df_filtered_50,
  file = "combined_eic_df_filtered_50_metabolites.rds"
)

# ============================================================
# Function: make integrated-area barplot for one metabolite
# ============================================================

plot_metabolite_barplot <- function(met_name) {
  
  met_df <- combined_eic_df_filtered_50 %>%
    filter(Metabolite == met_name)
  ...
  met_shifted <- met_df %>%
    left_join(
      met_peak %>%
        select(Wash, Replicate, scan_diff),
      by = c("Wash", "Replicate")
    ) %>%
    mutate(
      adjusted_scan = scan - scan_diff
  ...
  integrated_filtered <- integrated_data %>%
    group_by(Wash, Replicate) %>%
    filter(
      !(Intensity_Type == "intensity" & duplicated(Intensity_Type))
    ) %>%
    ungroup() %>%
    mutate(
      Compound = if_else(
  ...
  error_data <- integrated_filtered %>%
    filter(Wash != "PPT") %>%
    droplevels() %>%
    group_by(Wash, Compound) %>%
    summarise(
  ...
  ymax <- max(error_data$Mean + error_data$SE, na.rm = TRUE)
  ...
  p <- ggplot(
    ...
    labs(
      title = paste0(met_name, " integrated area"),
      x = NULL,
      y = "Integrated Area"
    ) +
    theme_minimal(base_size = 14) +
...
# ============================================================
# Export all 50 metabolite barplots to PDF
# ============================================================

barplot_pdf <- paste0(
  "C:/Users/DanielMalheiro/Downloads/Barplots_50_Metabolites_",
  format(Sys.time(), "%Y%m%d_%H%M%S"),
  ".pdf"
)
...
for (met in unique(targets$Metabolite)) {
  
  message("Plotting: ", met)
  
  p <- plot_metabolite_barplot(met)
...
## For all compound in the filtered feature list

```{r}
# ============================================================
# Extract all metabolites from feature_info and plot by PCI-SILS
# ============================================================
...
# ============================================================
# Create targets from feature_info
# ============================================================

targets <- feature_info %>%
  ungroup() %>%
  select(Feature, Name, mz, rt) %>%
  distinct() %>%
  transmute(
...
extract_all_targets_from_file <- function(file_name, targets, ppm, rt_window,
  ...
  purrr::pmap_dfr(
    targets,
    function(Feature, Metabolite, target_mz, rt_expected) {
...
# ============================================================
# Extract or load cached EICs
# ============================================================

cache_file <- "combined_eic_feature_info_metabolites.rds"

if (file.exists(cache_file)) {
  
  message("Loading cached EIC data from: ", cache_file)
  combined_eic_df_filtered <- readRDS(cache_file)
  
} else {
  
  message("Cached EIC data not found. Extracting...")
  ...
  saveRDS(combined_eic_df_filtered, cache_file)
  
  message("Saved EIC data to: ", cache_file)
}

# ============================================================
# Function: make integrated-area barplot for one metabolite
# Facets are now PCI-SILS instead of Wash
# ============================================================

plot_metabolite_barplot <- function(met_name) {
  
  met_df <- combined_eic_df_filtered %>%
    filter(Metabolite == met_name)
  ...
  integrated_filtered <- integrated_data %>%
    ...
    filter(
      Intensity_Type != "corrected_intensity" | !is.na(Compound)
    ) %>%
    mutate(
      Compound = factor(
        Compound,
...
# ============================================================
# Export all metabolite barplots to PDF
# ============================================================

barplot_pdf <- paste0(
  "C:/Users/DanielMalheiro/Downloads/Barplots_feature_info_metabolites_by_PCI_SILS_",
  format(Sys.time(), "%Y%m%d_%H%M%S"),
  ".pdf"
)
...
# Save results
saveRDS(eic_list2, file = (here("Data","eic_list2.rds")))


# saveRDS(eic_list, file = "eic_list.rds")

eic_list2 <- readRDS(here("Data","eic_list2.rds"))

# Combine all EICs into a single data frame
combined_eic_df <- bind_rows(eic_list2)

# Filter data to the retention time between 2 and 4 minutes
combined_eic_df_filtered <- combined_eic_df %>%
  filter(rt >= 2 & rt <= 4) %>%
    separate(
    file_name,
...
barplot_compounds_with_error_leu <- ggplot(
  ...
  aes(
    x = 3,
    y = Inf,
    label = paste0("PCI-SILS RSD = ", round(RSD_all_washes, 1), "%")
  ),
  inherit.aes = FALSE,
  vjust = 1.3,
  #fontface = "bold",
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "SI_XX.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "SI_XX.tiff"),
  plot = combined_mef_plot_leu,
  width = 22,
  height = 20,
  units = "cm",
