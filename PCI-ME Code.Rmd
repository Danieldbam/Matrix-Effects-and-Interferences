...
file_info$label <- factor(
  file_info$label,
  levels = c("60K", "120K", "240K")
)


#---------------------------------------------
# 2. Function to extract selected mass spectra
#---------------------------------------------
  ...
  spectra_list <- lapply(seq_along(scans), function(i) {
    ...
    df %>%
      dplyr::mutate(
        sample = label,
        scan = factor(
          paste0("Mass spectrum ", i),
          levels = paste0("Mass spectrum ", 1:4)
        )
      )
  })
...
mid_mz <- mean(target_masses)


# Half-width is measured on the outer flank only
# (the side facing away from the other peak) and doubled,
# so partial overlap cannot widen it.
...
get_resolution <- function(df, target_mz) {
  ...
  # Apex sitting on the edge means the peak
  # has coalesced with its neighbour
...
write_csv(
  ...
  here(
    "Data",
    "Figure_SI-14_resolution.csv"
  )
)


#---------------------------------------------
# 5. Effective resolving power labels
#---------------------------------------------

# Labels: one per resolved peak,
# placed beside its dashed line
...
peak_labels <- resolution_table %>%
  ...
  dplyr::filter(
    !paste(
      ...
      c(
        "240K Mass spectrum 3 glucose",
        "240K Mass spectrum 4 glucose"
      )
  ) %>%
  mutate(
    x = target +
...
spectrum_glucose <- ggplot(
  ...
  # Color by nominal resolving power
  geom_line(
    aes(
      color = sample,
      group = interaction(
  ...
  # Colors for each nominal resolving power
  scale_color_manual(
    values = c(
      "60K" = "#1f78b4",
      "120K" = "#33a02c",
