...
```{r global-theme}
# -------------------------------------------------------------------------
# Global plotting theme and colour palette
# Applied consistently across all figures
# Does NOT affect data or statistics
# -------------------------------------------------------------------------

theme_pci_me <- function(base_size = 11) {
...
Figure.2 <- tic_df %>% 
  ...
  labs(
    ...
    y = "Total Ion Current (offset for clarity)"
  ) +
...
kable(
  literature_table,
  format = "html",
  caption = "Table SI-1. Overview of different flowrates, analytes and analyte concentrations used for PCI experiments in literature."
) %>%
...
kable(
  wash_means_pub,
  caption = "Table SI-2. Contaminant intensity across PLR wash conditions. Values shown as mean and standard deviation for each homologue series ID.",
...
ggsave(
  filename = file.path(output_dir, "Figure SI-3 Article.png"),
  plot = allinter,
  width = 20, height = 20, units = "cm", dpi = 600
)

ggsave(
  filename = file.path(output_dir, "Figure SI-3 Article.tiff"),
  plot = allinter,
...
ggsave(
  filename = file.path(output_dir, "Figure SI-4 Article.png"),
  plot = box_sig_plot,
  width = 20, height = 20, units = "cm", dpi = 600
)

ggsave(
  filename = file.path(output_dir, "Figure SI-4 Article.tiff"),
  plot = box_sig_plot,
  width = 20, height = 20, units = "cm",
  dpi = 600, device = "tiff", compression = "lzw"
)
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure SI-5 Article.png"),
  plot = pub_ready_plot,
  width = 20,
  height = 10,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure SI-5 Article.tiff"),
  plot = pub_ready_plot,
  width = 20,
  height = 10,
  units = "cm",
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure SI-6 Article.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure SI-6 Article.tiff"),
  plot = Ioninjection_plot,
  width = 20,
  height = 10,
  units = "cm",
...
ggsave(
  filename = file.path(output_dir, "Figure SI-7 Article.png"),
...
ggsave(
  filename = file.path(output_dir, "Figure SI-7 Article.tiff"),
  plot = chromatogram_overlay_scaled,
  width = 20,
  height = 10,
  units = "cm",
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure SI-8. Article.png"),
  plot = plot_all,
  width = 20,
  height = 15,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure SI-8. Article.tiff"),
  plot = plot_all,
  width = 20,
  height = 15,
  units = "cm",
...
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure SI-9. Article.png"),
  plot = WC_plot,
  width = 20,
  height = 25,
  units = "cm",
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure SI-9. Article.tiff"),
  plot = WC_plot,
  width = 20,
  height = 25,
  units = "cm",
...
\`\`\`{r} library(readxl) library(writexl)

write_files_to_excel \<- function(files, sheet_names = NULL, out_file =
"combined_output.xlsx") {

\# If sheet names not provided, use file names without extension if
(is.null(sheet_names)) { sheet_names \<-
tools::file_path_sans_ext(basename(files)) }

\# Check sheet names length if (length(sheet_names) != length(files)) {
stop("sheet_names must be the same length as files.") }

df_list \<- list()

for (i in seq_along(files)) { file \<- files[i] ext \<-
tools::file_ext(file)
...
write_xlsx(df_list, out_file) message("Excel file written: ", out_file)
}

setwd("C:/Users/DanielMalheiro/OneDrive - Clinical
Microbiomics/Documents/R scripts")

files \<- c( "merged_class_df.xlsx", "LC_Final
results_ver12-Exp7-PCI.xlsx", "PCI-peaksforR.xlsx" )

# Optional custom sheet names

sheet_names \<- c("MetaResults", "Peaks","Homologue Series")

write_files_to_excel(files, sheet_names, out_file = "Supplimentary
Information_Excel_Files.xlsx")
