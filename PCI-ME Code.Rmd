...
2.  **Supplementary Figures** (Figure SI-1 - SI-20)
...
-   **Supplementary Figures**
    ...
    -   Figure SI-3. Leucine-d10 EIC in solvent blanks
    -   Figure SI-4. All compounds, positive mode
    -   Figure SI-5. All compounds, negative mode
    -   Figure SI-6. Contaminant MS (image)
    -   Figure SI-7. Contaminant ions MS
    -   Figure SI-8. Summed EICs of contaminants
    -   Figure SI-9. Contaminant and matrix-effect overlap
    -   Figure SI-10. Homologue series map
    -   Figure SI-11. Effect of PLR wash intensity on contaminant signals
    -   Figure SI-12. Injection time
    -   Figure SI-13. EIC of Acetyl-Carnitine and Glucose-13C6
    -   Figure SI-14. Resolution figure
    -   Figure SI-15. Dilution MS spectra
    -   Figure SI-16. Isoleucine spike
    -   Figure SI-17. Training and validation on Tryptophan
    -   Figure SI-18. RSD change versus retention time
    -   Figure SI-19. Improved and worsened features
    -   Figure SI-20. Individual worsened features

### Reproducibility

```{r chunk-reproducibility, eval=FALSE}
## Run once from the project root before reproducing the analysis.
## The manuscript analysis was developed with R 4.3.3.
...
# Open the project .Rproj before running this document. here() then resolves
# project-relative paths without requiring setwd().
#
# Path priority:
#   1. An explicit environment variable.
#   2. A downloaded MetaboLights study directory.
#   3. The documented project-relative directory.
#   4. Daniel's temporary local directory (remove after publication testing).

# -------------------------------------------------------------------------
# Configurable output directory
# -------------------------------------------------------------------------
# Set PCI_OUTPUT_DIR to write figures elsewhere. By default, all generated
# figures are written to <project>/Figures.
output_dir <- Sys.getenv("PCI_OUTPUT_DIR", unset = here("Figures"))
results_dir <- Sys.getenv("PCI_RESULTS_DIR", unset = here("Results"))

dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

# -------------------------------------------------------------------------
# Flexible mzML-directory discovery
# -------------------------------------------------------------------------
# After publication, download and extract the complete MetaboLights study,
# then set METABOLIGHTS_STUDY_DIR to its top-level directory. The study
# accession and extraction-directory name are deliberately not hard-coded.
metabolights_study_dir <- Sys.getenv("METABOLIGHTS_STUDY_DIR", unset = "")

has_mzml_files <- function(path) {
...
find_mzml_directory <- function(root, include_pattern = NULL,
  ...
  if (!is.null(include_pattern)) {
    candidate_dirs <- candidate_dirs[grepl(
      include_pattern,
      candidate_dirs,
      ignore.case = TRUE
    )]
  ...
  # Prefer the shallowest matching folder for predictable selection.
  candidate_dirs[order(nchar(candidate_dirs))][1]
}

first_existing_directory <- function(paths) {
  paths <- unique(paths[!is.na(paths) & nzchar(paths)])
  match <- paths[dir.exists(paths)]
  if (length(match) == 0L) NA_character_ else match[1]
}

# Main PCI mzML dataset. PCI_MZML_DIR always has highest priority.
main_metabolights_mzml <- find_mzml_directory(
  metabolights_study_dir,
  exclude_pattern = "gradient|dilution"
)

mzml_folder <- first_existing_directory(c(
  Sys.getenv("PCI_MZML_DIR", unset = ""),
  main_metabolights_mzml,
  here("Data", "mzML-PAN"),
  "C:/Users/DanielMalheiro/Downloads/Exp7-241008/mzML-PAN"
))

# Gradient/dilution mzML dataset. PCI_DILUTION_MZML_DIR has highest priority.
dilution_metabolights_mzml <- find_mzml_directory(
  metabolights_study_dir,
  include_pattern = "gradient|dilution"
)

dilution_mzml_folder <- first_existing_directory(c(
  Sys.getenv("PCI_DILUTION_MZML_DIR", unset = ""),
  dilution_metabolights_mzml,
  here("Data", "Gradient-and-Dilution", "mzML-PAN"),
  "C:/Users/DanielMalheiro/Downloads/Exp7-Gradient and Dilution/mzML-PAN"
))

# Print the resolved locations so the analysis log records exactly which
# inputs and output directory were used.
message("Figure output directory: ", normalizePath(output_dir, mustWork = FALSE))
message("Results output directory: ", normalizePath(results_dir, mustWork = FALSE))
message("Main mzML directory: ", ifelse(is.na(mzml_folder), "NOT FOUND", mzml_folder))
message(
  "Dilution mzML directory: ",
  ifelse(is.na(dilution_mzml_folder), "NOT FOUND", dilution_mzml_folder)
)

# Reusable checks provide an informative error at the point a required input
# is used, rather than failing later with a cryptic file-not-found message.
require_file <- function(path, description = "Required input file") {
  if (!file.exists(path)) {
    stop(description, " was not found: ", normalizePath(path, mustWork = FALSE), call. = FALSE)
  }
  path
}

require_dir <- function(path, description = "Required input directory") {
  if (!dir.exists(path)) {
    stop(
      description, " was not found: ", normalizePath(path, mustWork = FALSE),
      "\nSet the corresponding environment variable or create the documented project folder.",
      call. = FALSE
    )
  }
  path
...
MEvsEIC_AllRep <- (
  ...
  plot_annotation(
    title = "Including contaminated solvent blanks (n = 8)",
    theme = theme(
...
MEvsEIC_CleanRep <- (
  ...
  plot_layout(heights = c(2, 2, 2)) +
  plot_annotation(
    title = "Including non-contaminated solvent blanks (n = 3)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,
        face = "bold",
...
# ================================
# 3. Add A-F labels to the existing plots
# ================================

MEvsEIC_AllRep_tagged <- MEvsEIC_AllRep +
  plot_annotation(
...
MEvsEIC_CleanRep_tagged <- MEvsEIC_CleanRep +
  plot_annotation(
...
# ================================
# 4. Combine the EXISTING titled plots
# ================================

main_figure <- plot_grid(
  MEvsEIC_AllRep_tagged,
  MEvsEIC_CleanRep_tagged,
  ncol = 2,
  rel_widths = c(1, 1)
)
...
## Figure 3. Homologue Series

### Data prep: build merged_df used by Figure 3 and the negative-mode summary

```{r homologue-dataprep}
# ---- Data prep: build merged_df used by Figure 3 and the negative-mode summary ----
Sup_info <- here("Data","Supplementary Tables.xlsx")

<<<<<<< HEAD
data3 <- read_excel(Sup_info, sheet = "Table SI-5",
  skip = 1) 
=======
data3<- data3 <- read_excel(Sup_info, sheet = "Table SI-5")
>>>>>>> bcc2fd54737970cabe4e345ed0756d4b575907df
...
merged_df <- difference_left_join(
  ...
  data3 %>% 
    ...
    select(
      mz, rt, Formula, "Annotation level", "Name (RefMet Update)", Adduct, MS2,
      Polarity,
      #identifier,
      #"Compound #",
...
Sup_info <- here("Data","Supplementary Tables.xlsx")
data4<- read_excel(Sup_info, sheet = "Table SI-6", skip = 1)
...
long_data_dilu <- combined_data_dilu %>%
  filter(Date == "241009",
         Compound %in% c(
           #"Indole-d6",
           "Leucine-d10",
...
data_meta <- read_excel(Sup_info, sheet = "Table SI-5", skip = 1) %>%
=======
data4 <- read_excel(Sup_info, sheet = "Table SI-6")

data_meta <- read_excel(Sup_info, sheet = "Table SI-5") %>%
>>>>>>> bcc2fd54737970cabe4e345ed0756d4b575907df
  mutate(
    Charge = case_when(
      Polarity == "POS" ~ 1,
...
rsd_per_wash <- nist_data %>%
  ...
  left_join(
    data_meta %>% select(`id`, "Name (RefMet Update)", rt, mz, Adduct, Charge),
    by = c("Feature" = "id")
  ) %>%
  filter(Charge == 1)
...
label_improved <- paste0(
  "ΔRSD ≥ ", RSD_CUTOFF, "%  (n = ", n_improved, ")"
)

label_worsened <- paste0(
  "ΔRSD ≤ -", RSD_CUTOFF, "%  (n = ", n_worsened, ")"
)
...
rsd_per_wash_neg <- nist_data %>%
  ...
  left_join(
    data_meta %>%
      select(id, "Name (RefMet Update)", rt, mz, Adduct, Charge),
    by = c("Feature" = "id")
  ) %>%
  filter(Charge == -1)
...
n_improved_neg <- sum(
  rsd_delta_rt_neg$Category == "Improved",
  na.rm = TRUE
)

n_worsened_neg <- sum(
  rsd_delta_rt_neg$Category == "Worsened",
  na.rm = TRUE
)

label_improved_neg <- paste0(
  "\u0394RSD \u2265 ",
  RSD_CUTOFF,
  "%  (n = ",
  n_improved_neg,
...
>>>>>>> 25f78d9b95e949ec25b61af96a57f4fbb439ef18
)
...
## Figure SI-4 & SI-5. All compounds, positive and negative modes
...
```{r figure-si-7-plot-detail}
# ---- Data and Plot 2: ----


# Load data pre-extracted for the contaminant-ion analysis.
...
```{r figure-si-7-export}
# Save PNG
ggsave(
  filename = file.path(output_dir, "Figure_SI-7.png"),
...
# Save TIFF
ggsave(
  filename = file.path(output_dir, "Figure_SI-7.tiff"),
  plot = contaminent_blanks,
  width = 20,
  height = 15,
  units = "cm",
...
#---------------------------------------------
#---------------------------------------------
# 4. Effective mass resolution (m/Δm at FWHM)
#---------------------------------------------
target_masses <- c(204.1173, 204.1230)   # glucose-13C6 , acetyl-carnitine
mid_mz <- mean(target_masses)            # split point between the two peaks

# Half-width is measured on the outer flank only (the side facing away from the
# other peak) and doubled, so partial overlap cannot widen it.
get_resolution <- function(df, target_mz) {
  
  df <- df %>% arrange(mz)
  
  if (target_mz < mid_mz) {
    df <- df %>% dplyr::filter(mz <= mid_mz)
  } else {
    df <- df %>% dplyr::filter(mz >= mid_mz)
  }
  
  apex <- which.max(df$intensity)
  half <- df$intensity[apex] / 2
  
  # Apex sitting on the edge means the peak has coalesced with its neighbour
  if (apex == 1 || apex == nrow(df)) {
    return(data.frame(target = target_mz, apex_mz = NA, fwhm = NA, R = NA))
  }
  
  flank <- if (target_mz < mid_mz) df[1:apex, ] else df[apex:nrow(df), ]
  mz_half <- approx(flank$intensity, flank$mz, xout = half, ties = min)$y
  fwhm <- 2 * abs(df$mz[apex] - mz_half)
  
  data.frame(target = target_mz, apex_mz = df$mz[apex],
             fwhm = fwhm, R = df$mz[apex] / fwhm)
}

resolution_table <- combined_spectra %>%
  group_by(sample, scan) %>%
  group_modify(~ purrr::map_dfr(target_masses, get_resolution, df = .x)) %>%
  ungroup()

print(resolution_table, n = Inf)

write_csv(resolution_table, here("Data", "Figure_SI-14_resolution.csv"))

# Labels: one per resolved peak, placed beside its dashed line
peak_labels <- resolution_table %>%
  dplyr::filter(!is.na(R)) %>%
  mutate(peak = ifelse(target < mid_mz, "glucose", "carnitine")) %>%
  dplyr::filter(!paste(sample, scan, peak) %in%
                  c("240K Scan 3 glucose", "240K Scan 4 glucose")) %>%
  mutate(
    x = target + ifelse(target < mid_mz, -0.0010, 0.0010),
    y = 1.6e6,
    label = paste0("R = ", format(round(R, -3), big.mark = ",", trim = TRUE))
  )
...
spectrum_glucose <- ggplot(combined_spectra, aes(x = mz, y = intensity)) +
  ...
  # ⭐ Effective resolving power beside each peak
  geom_text(
    data = peak_labels,
    aes(x = x, y = y, label = label),
    inherit.aes = FALSE,
    angle = 90, hjust = 0.5, size = 3, color = "black"
  ) +
  
  facet_grid(sample ~ scan, scales = "free_x") +
  
  coord_cartesian(xlim = c(204.11, 204.13), ylim = c(0, 2.5e6)) +
...
# Save
ggsave(
  filename = file.path(output_dir, "Figure_SI-14.png"),
  plot = spectrum_glucose,
  width = 20, height = 20, units = "cm", dpi = 1000
)

ggsave(
  filename = file.path(output_dir, "Figure_SI-14.tiff"),
  plot = spectrum_glucose,
  width = 20, height = 20, units = "cm",
  dpi = 1000, device = "tiff", compression = "lzw"
)
...
```{r figure-si-15-extract-nowash-ms}
# ============================================================
...
raw_folder <- dilution_mzml_folder
...
```{r figure-si-16-plots}
# -----------------------------
# REBUILD the isoleucine long-format df IN THIS SECTION.
# (These steps were missing, so the plot was using Figure 6's stale
#  combined_eic_df_filtered_long instead of the isoleucine data.)
# -----------------------------

# Step 1: per Wash/Replicate, take the scan at the intensity maximum (for alignment)
combined_eic_df_filtered2 <- combined_eic_df_filtered %>%
  group_by(Wash, Replicate) %>%
  filter(rt == rt[which.max(intensity)]) %>%
  ungroup()

# Step 2: find the closest MEF Time/Scan to each peak rt -> per-run scan offset
combined_eic_df_filtered2 <- combined_eic_df_filtered2 %>%
  mutate(
    closest_time = purrr::map_dbl(rt,           ~ matrix_effect_data$Times[which.min(abs(matrix_effect_data$Times - .))]),
    closest_scan = purrr::map_dbl(closest_time, ~ matrix_effect_data$Scan [which.min(abs(matrix_effect_data$Times - .))]),
...
# Step 3: apply the scan shift to the full EIC
combined_eic_df_with_shift <- combined_eic_df_filtered %>%
  left_join(
    combined_eic_df_filtered2 %>% select(Wash, Replicate, scan_diff),
    by = c("Wash", "Replicate")
  ) %>%
  mutate(adjusted_scan = scan - scan_diff)

# Step 4: join each PCI's MEF (by aligned scan) -> corrected intensity + Compound labels
combined_eic_df_with_shift <- combined_eic_df_with_shift %>%
  left_join(
  ...
  mutate(
    ...
    Wash = factor(Wash, levels = c("22x", "08x", "04x", "NoWash", "PPT")),
    Wash = recode(Wash,
      "22x" = "PLR 22x wash", "08x" = "PLR 8x wash", "04x" = "PLR 4x wash",
      "NoWash" = "PLR unwashed", "PPT" = "PPT"),
    Compound_plot = recode(Compound,
      "Average"         = "Average",
      "Tryptophan-d5"   = "Tryptophan-d[5]",
      "Leucine-d10"     = "Leucine-d[10]",
      "Glucose-13C6"    = "Glucose-{}^{13}*C[6]",
...
compound_plot_levels <- c(
  "Tryptophan-d[5]", "Leucine-d[10]", "Glucose-{}^{13}*C[6]",
  "Stearic~acid-d[35]", "Indole-d[6]", "Carnitine-d[9]"
)

# Step 5: long format (uncorrected + corrected), averaged per rt
combined_eic_df_filtered_long <- combined_eic_df_with_shift %>%
  pivot_longer(
    cols = c(intensity, corrected_intensity),
    names_to = "Intensity_Type",
  ...
  group_by(Wash, rt, Matrix_Effect_Factor, Sample, Replicate,
           Intensity_Type, Sequence, Compound, Compound_plot) %>%
  summarise(Average_Intensity = mean(Intensity_Value, na.rm = TRUE), .groups = "drop") %>%
  mutate(Compound_plot = factor(Compound_plot, levels = compound_plot_levels))


# -----------------------------
# Build the subset that is actually plotted, THEN scale the MEF to it
# (scaling must use the plotted window, not the whole dataset)
# -----------------------------
plot_leu <- combined_eic_df_filtered_long %>%
  filter(between(rt, 2.6, 3), Wash == "PLR 4x wash") %>%
  mutate(
    Intensity_Type = recode(
      Intensity_Type,
      intensity           = "Uncorrected Tryptophan Intensity",
    ...
    Intensity_Type = factor(
      Intensity_Type,
      levels = c("Uncorrected Tryptophan Intensity",
                 "Corrected Tryptophan Intensity",
                 "Matrix Factor")
    )
  )

# axis limit must cover BOTH traces; MEF is scaled to the UNCORRECTED peak
int_max_all    <- max(plot_leu$Average_Intensity, na.rm = TRUE)
int_max_uncorr <- max(plot_leu$Average_Intensity[
  plot_leu$Intensity_Type == "Uncorrected Tryptophan Intensity"], na.rm = TRUE)
mef_max        <- max(plot_leu$Matrix_Effect_Factor, na.rm = TRUE)

# MEF * scaling_factor -> peaks at the uncorrected-intensity peak (comparable heights)
scaling_factor <- int_max_uncorr #/ mef_max

# -----------------------------
# Step 7: Plot
# -----------------------------
All_plot_EIC_long_leu <- plot_leu %>%
  ggplot() +

  # Intensity traces
  geom_line(
    aes(x = rt, y = Average_Intensity, color = Intensity_Type, group = Intensity_Type),
    linewidth = 0.5
  ) +

  # MEF trace on the secondary scale
  geom_line(
    aes(x = rt, y = Matrix_Effect_Factor * scaling_factor, color = "Matrix Factor"),
    linewidth = 0.5
  ) +

  # MEF = 1 reference (no matrix effect)
  geom_hline(yintercept = 1.0 * scaling_factor, color = "red", linetype = "dashed") +
  ...
  scale_y_continuous(
    name    = "Intensity",
    limits  = c(0, int_max_all * 1.05),
    labels  = scales::scientific,
    sec.axis = sec_axis(
      transform = ~ . / scaling_factor,   # ggplot2 4.0: 'transform', not 'trans'
      name      = "Matrix Factor"
    )
  ) +
  ...
  labs(x = "Retention Time (min)", y = "Intensity", title = "A") +
  ...
  theme(
    ...
    axis.ticks        = element_line(color = "black"),
    plot.title.position = "plot",
    plot.title        = element_text(hjust = 0, vjust = 1, face = "bold", size = 12)
  )
...
```{r figure-si-17-analysis}
# ============================================================
# Per-feature RSD across correction methods
# Training versus validation
# Bars ordered best -> worst within each correction
# Run after the Figure 7 training/validation chunk
# Requires: split_df
# ============================================================

# Helper for ordering bars separately within each facet
...
# Nice correction-method labels
pretty_correction <- c(
  uncorrected      = "Uncorrected",
  corrected_mean   = "Best mean",
  corrected_median = "Best median",
  corrected_min    = "Best minimum"
)

# ============================================================
# Calculate RSD for each PCI, correction type, and dataset
# ============================================================

rsd_feature <- split_df %>%
  group_by(
    Feature,
    PCI,
    CorrectionType,
  ...
  # The uncorrected values are repeated for every PCI.
  # Keep only one uncorrected reference row.
  filter(
    CorrectionType != "uncorrected" |
      (
        CorrectionType == "uncorrected" &
  ...
  mutate(
    CorrectionType = factor(
      CorrectionType,
      levels = names(pretty_correction),
      labels = pretty_correction
    ),
    ...
    # Assign a separate fill-group name to the uncorrected bar
    fill_group = if_else(
      CorrectionType == "Uncorrected",
      "Uncorrected",
      as.character(PCI)
...
plot_rsd_feature <- ggplot(
  ...
  # Rows = training and validation
  # Columns = correction methods
  facet_grid(
    Set ~ CorrectionType,
    scales = "free_x",
    space = "free_x"
  ...
  # Remove the artificial facet-ordering suffix
  scale_x_discrete(
    labels = function(x) {
      ...
      # Do not show a PCI label beneath the uncorrected bar
      lab[
        grepl(
          "___Uncorrected$",
          x
  ...
  # Use the Dark2 RColorBrewer palette
  scale_fill_brewer(
    palette = "Dark2"
  ) +

  # Add space above the bars for RSD labels
  scale_y_continuous(
    expand = expansion(
      mult = c(0, 0.15)
    )
  ...
  theme(
    ...
    # Allows the full "Uncorrected" facet label to be visible
    strip.clip = "off",
...
## Change in Standard Names using Refmet

```{r}
# ============================================================
# OPTIONAL RefMet name / InChIKey cross-check
# ============================================================
# FALSE skips all imports, online requests, and exports in this block.
# TRUE sends the feature names to the RefMet endpoint used in your original code.
# This block does not write back to Supplementary Tables.xlsx.

run_refmet_check <- FALSE

# NULL finds a unique tab labelled Table SI-5 or SI-5, optionally with a title.
# Alternatively, enter the exact tab name, e.g. "Table SI-5 - Feature List".
feature_sheet <- NULL

# NULL uses the existing results_dir, or the project's results/ directory.
refmet_output_dir <- NULL

# Protect a previously saved RefMet review workbook by default.
overwrite_refmet_output <- FALSE

if (isTRUE(run_refmet_check)) {

  # Check dependencies only when this optional step is enabled.
  required_packages <- c(
    "here", "readxl", "dplyr", "stringr", "stringi",
    "tibble", "purrr", "curl", "writexl"
  )
  ...
  # Preserve the parent workflow's results directory when it is available.
  if (is.null(refmet_output_dir)) {
    refmet_output_dir <- if (exists("results_dir", inherits = TRUE)) {
      get("results_dir", inherits = TRUE)
    } else {
      here::here("results")
    }
  }
  if (!is.character(refmet_output_dir) || length(refmet_output_dir) != 1L ||
      is.na(refmet_output_dir) || !nzchar(refmet_output_dir)) {
    stop("refmet_output_dir must be one non-empty directory path.", call. = FALSE)
  }
  dir.create(refmet_output_dir, recursive = TRUE, showWarnings = FALSE)
  if (!dir.exists(refmet_output_dir) || file.access(refmet_output_dir, 2L) != 0L) {
    stop("Output directory is not writable: ", refmet_output_dir, call. = FALSE)
  }

  output_file <- file.path(refmet_output_dir, "RefMet_name_crosscheck.xlsx")
  if (file.exists(output_file) && !isTRUE(overwrite_refmet_output)) {
    stop(
      "An existing RefMet output has been retained: ", output_file, "\n",
      "Set overwrite_refmet_output <- TRUE to replace it deliberately.",
      call. = FALSE
    )
  }

  # ============================================================
  # 1. Read names and InChIKeys from Excel
  # ============================================================

  Sup_info <- here::here("Data", "Supplementary Tables.xlsx")

  if (!file.exists(Sup_info)) {
    stop("Workbook not found: ", Sup_info, call. = FALSE)
  }

  # Use the table LABEL, not the fifth worksheet position.
  # An exact sheet name supplied in the settings overrides automatic selection.
  available_sheets <- readxl::excel_sheets(Sup_info)

  if (is.null(feature_sheet)) {
    # Normalize dash characters only for matching; read with the original tab name.
    normalized_sheets <- trimws(gsub(
      "[\u2010-\u2015\u2212]", "-", available_sheets, perl = TRUE
    ))

    candidates <- available_sheets[grepl(
      "^(Table[[:space:]]+)?SI[[:space:]]*-[[:space:]]*0*5($|[[:space:]_:().-])",
      normalized_sheets,
      ignore.case = TRUE,
      perl = TRUE
    )]
    ...
    if (length(candidates) != 1L) {
      stop(
        "Could not identify exactly one Table SI-5 worksheet.\n",
        "Set feature_sheet to the exact Feature List tab name.\n",
        "Available sheets: ", paste(available_sheets, collapse = "; "),
        call. = FALSE
      )
    }
    feature_sheet <- candidates[[1]]
  }

  if (!is.character(feature_sheet) || length(feature_sheet) != 1L ||
      is.na(feature_sheet) || !(feature_sheet %in% available_sheets)) {
    stop(
      "feature_sheet must be an exact worksheet name.\n",
      "Available sheets: ", paste(available_sheets, collapse = "; "),
      call. = FALSE
    )
  }

  message("Reading Feature List from: ", feature_sheet)
  feature_input <- readxl::read_excel(Sup_info, sheet = feature_sheet)

  missing_columns <- setdiff(c("Name", "InChI Key"), names(feature_input))
  if (length(missing_columns) > 0L) {
    stop(
      "Required columns missing from ", feature_sheet, ": ",
      paste(missing_columns, collapse = ", "), ".\n",
      "Check the column names and header row. This import does not skip title rows.",
      call. = FALSE
    )
  }
  if (nrow(feature_input) == 0L) {
    stop("The Feature List worksheet contains no data rows.", call. = FALSE)
  }

  DF_all <- feature_input %>%
    dplyr::select(
      Name,
      Excel_InChIKey = `InChI Key`
    ) %>%
    dplyr::mutate(
      Row_ID = dplyr::row_number(),
      Name = as.character(Name),
      Excel_InChIKey = as.character(Excel_InChIKey),
      Excel_InChIKey_clean = Excel_InChIKey %>%
        stringr::str_trim() %>%
  ...
  DF_all <- DF_all %>%
    dplyr::mutate(
      Batch = ceiling(Row_ID / batch_size)
    )
  ...
  refmet_url <- paste0(
    ...
    "databases/refmet/name_to_refmet_new_minID.php"
  )

  # ============================================================
  # 4. Function to query one batch
  # ============================================================

  query_one_batch <- function(batch_df) {

    batch_number <- unique(batch_df$Batch)
    ...
    if (nrow(batch_valid) == 0) {
      return(
        tibble::tibble(
          Row_ID = character(),
          `Input name` = character(),
          `Standardized name` = character(),
          Formula = character(),
          `Exact mass` = character(),
          ...
          RefMet_ID = character(),
          Batch = character(),
          Batch_status = character()
        )
      )
    }

    mets <- stringi::stri_join(
      batch_valid$Name,
      collapse = "\n"
    )
    ...
    result <- tryCatch(
      {
        ...
        response_text <- rawToChar(req$content)

        if (
          grepl(
            "Internal error|SQL execution failed|<html|<br",
            response_text,
            ignore.case = TRUE
          )
        ) {
          stop("RefMet server returned an error")
        }
        ...
        # Remove duplicate uppercase ID columns if returned
        output <- output %>%
          dplyr::select(
            -dplyr::any_of(
              c(
        ...
        # Match RefMet results back to submitted rows by input name
        output <- output %>%
          dplyr::left_join(
            batch_valid %>%
              dplyr::select(
      ...
      error = function(e) {
        ...
        batch_valid %>%
          dplyr::transmute(
            Row_ID = as.character(Row_ID),
            `Input name` = as.character(Name),
            `Standardized name` = NA_character_,
            Formula = NA_character_,
            `Exact mass` = NA_character_,
            `Super class` = NA_character_,
            `Main class` = NA_character_,
            `Sub class` = NA_character_,
  ...
  refmet_all <- purrr::map_dfr(
    batches,
    query_one_batch
  )

  refmet_all <- refmet_all %>%
    dplyr::mutate(
      Row_ID = as.numeric(Row_ID)
    )

  # ============================================================
  # 6. Clean RefMet InChIKeys
  # ============================================================

  refmet_all <- refmet_all %>%
    dplyr::mutate(
      RefMet_InChIKey_clean = INCHI_KEY %>%
        as.character() %>%
        stringr::str_trim() %>%
        stringr::str_to_upper() %>%
        dplyr::na_if("") %>%
  ...
  # ============================================================
  # 7. Add Excel values to every RefMet result
  # ============================================================

  refmet_matches <- refmet_all %>%
    dplyr::left_join(
      DF_all %>%
        dplyr::select(
          Row_ID,
          Name,
          Excel_InChIKey,
          Excel_InChIKey_clean
        ),
      by = "Row_ID"
    )

  # ============================================================
  # 8. Compare InChIKeys and score matches
  # ============================================================

  refmet_matches <- refmet_matches %>%
    dplyr::mutate(
      InChIKey_check = dplyr::case_when(
        Batch_status == "Failed" ~
          "RefMet request failed",

        is.na(Excel_InChIKey_clean) &
          is.na(RefMet_InChIKey_clean) ~
          "No InChIKey available",
        ...
        Excel_InChIKey_clean ==
          RefMet_InChIKey_clean ~
          "Full match",

        substr(Excel_InChIKey_clean, 1, 14) ==
          substr(RefMet_InChIKey_clean, 1, 14) ~
          "Connectivity match",
      ...
      Match_score = dplyr::case_when(
        InChIKey_check == "Full match" ~ 1L,
        InChIKey_check == "Connectivity match" ~ 2L,
        InChIKey_check == "No Excel InChIKey" ~ 3L,
        InChIKey_check == "No RefMet InChIKey" ~ 4L,
        InChIKey_check == "No InChIKey available" ~ 5L,
        InChIKey_check == "Mismatch" ~ 6L,
        InChIKey_check == "RefMet request failed" ~ 7L,
        TRUE ~ 8L
      )
    )

  # ============================================================
  # 9. Select best RefMet match for each Excel row
  # ============================================================

  refmet_best <- refmet_matches %>%
  ...
  # ============================================================
  # 10. Remove duplicate ID columns and rename clearly
  # ============================================================

  refmet_best <- refmet_best %>%
    dplyr::select(
      -dplyr::any_of(
        c(
          "PUBCHEM_CID",
    ...
    dplyr::rename(
      RefMet_Input_name = `Input name`,
      RefMet_name = `Standardized name`,
      RefMet_Formula = Formula,
      RefMet_Exact_mass = `Exact mass`,
      RefMet_Super_class = `Super class`,
      RefMet_Main_class = `Main class`,
      RefMet_Sub_class = `Sub class`,
      RefMet_InChIKey = INCHI_KEY,
      RefMet_PubChem_CID = PubChem_CID,
      RefMet_ChEBI_ID = ChEBI_ID,
      RefMet_HMDB_ID = HMDB_ID,
      RefMet_LipidMaps_ID = LM_ID,
      RefMet_KEGG_ID = KEGG_ID
    )

  # RefMet_ID already has the correct name and is kept unchanged

  # ============================================================
  # 11. Final result in original row order
  # ============================================================
  ...
  # ============================================================
  # 12. Create review tables
  # ============================================================

  manual_review <- result_all %>%
    dplyr::filter(
      InChIKey_check %in% c(
        "Connectivity match",
        "Mismatch",
  ...
  all_multiple_matches <- refmet_matches %>%
    dplyr::group_by(Row_ID) %>%
  ...
  cat(
    "\nRows with multiple RefMet matches:",
    dplyr::n_distinct(all_multiple_matches$Row_ID),
    "\n"
  )
  ...
  # output_file was checked before starting the RefMet requests.

  writexl::write_xlsx(
    list(
      Best_RefMet_match = result_all,
      Manual_review = manual_review,
      All_multiple_matches = all_multiple_matches
    ),
    output_file
  )
  ...
  message("RefMet cross-check skipped (run_refmet_check = FALSE).")
}
```
...
## Record the exact software environment used for this run.
sessionInfo()
```
