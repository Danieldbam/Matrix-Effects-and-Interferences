...
summary_table_1 <- filtered_data_1 %>%
  ...
  mutate(
    ...
    mutate(
   Wash = recode(Wash,
                  "22x" = "PLR 22x wash", #previously "PLR 22x wash"
                  "NoWash" = "PLR unwashed"), #previously "PLR unwashed"
...
# ===============================
# Step 4: Filter + relabel logic (idEMPOTENT & SAFE)
# ===============================
integrated_data_long_wide_filtered <- integrated_data_long_wide %>%
  
  # Keep ONE uncorrected (intensity) per Wash + Replicate
  group_by(Wash, Replicate) %>%
  filter(!(Intensity_Type == "intensity" & duplicated(Intensity_Type))) %>%
  ungroup() %>%
  
  # Define Uncorrected ONLY by Intensity_Type (never by Compound)
  mutate(
    Compound = if_else(
  ...
  # Keep:
  # - Uncorrected (from intensity)
  # - All corrected compounds
  filter(
...
# ===============================
# Step 6: Recode compounds for plotting (labels only)
# ===============================
...
barplot_compounds_with_error <- ggplot(
  ...
  expand = expansion(mult = c(0, 0.15))  # ⬅ 15% headroom at the top
) +
...
plot_df <- val_best_worsened %>%
  ...
  mutate(
    ...
    Feature_RT = factor(Feature_RT, levels = facet_levels),
      Treatment = factor(Treatment, levels = c("22x", "08x", "04x", "NoWash", "PPT")),
    Treatment = recode(
      Treatment,
      "22x"    = "PLR 22x wash",
      "08x"    = "PLR 8x wash",
