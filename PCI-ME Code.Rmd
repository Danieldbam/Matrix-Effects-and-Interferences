...
# ============================================================
# Export MF correction results to Supplementary Data
# ============================================================
Sup_info <- here("Data", "Supplementary Data.xlsx")
# Feature m/z and retention time, both polarities
feature_info <- bind_rows(
  rsd_per_wash15     %>% mutate(Polarity = "POS"),
  rsd_per_wash15_neg %>% mutate(Polarity = "NEG")
) %>%
  distinct(Polarity, Feature, Name, mz, rt)
# --- Sheet 1: one row per feature and correction strategy, with the RSDs ---
mf_correction_summary <- bind_rows(
  plot_df     %>% mutate(Polarity = "POS"),
  plot_df_neg %>% mutate(Polarity = "NEG")
) %>%
  left_join(feature_info, by = c("Polarity", "Feature")) %>%
  transmute(
    Polarity,
    Feature,
    Name,
    mz,
    RT_min                     = rt,
    Correction_strategy        = as.character(Type),
    Applied_correction         = Chosen,
    Applied_PCI_SILS           = Chosen_PCI,
    RSD_training_uncorrected   = train_rsd_uncorrected,
    RSD_training_corrected     = Best_train_RSD,
    RSD_validation_uncorrected = RSD_uncorrected,
    RSD_validation_corrected   = RSD_corrected,
    RSD_validation_selected    = RSD_val
  ) %>%
  arrange(Polarity, Feature, Correction_strategy)
# --- Sheet 2: uncorrected and corrected peak areas per replicate ---
mf_correction_areas <- bind_rows(
  result_long     %>% mutate(Polarity = "POS"),
  result_long_neg %>% mutate(Polarity = "NEG")
) %>%
  select(
    Polarity, Feature, Name, Wash, Replicate,
    PCI = Compound,
    mean_MF, median_MF, min_MF,
    Correction_Type, Corrected_Value
  ) %>%
  pivot_wider(
    names_from  = Correction_Type,
    values_from = Corrected_Value
  ) %>%
  left_join(
    feature_info %>% select(Polarity, Feature, mz, rt),
    by = c("Polarity", "Feature")
  ) %>%
  arrange(Polarity, Feature, Wash, Replicate, PCI)
# --- Write back, keeping every existing sheet ---
sheet_names <- readxl::excel_sheets(Sup_info)
sup_data <- lapply(sheet_names, function(s) read_excel(Sup_info, sheet = s))
names(sup_data) <- sheet_names
sup_data[["MF Correction"]]       <- mf_correction_summary
sup_data[["MF Correction Areas"]] <- mf_correction_areas
writexl::write_xlsx(sup_data, Sup_info)
cat("MF Correction:", nrow(mf_correction_summary), "rows\n")
cat("MF Correction Areas:", nrow(mf_correction_areas), "rows\n")
...
```{r}
# ============================================================
# RefMet conversion for all names
# InChIKey cross-check using Excel column "InChI Key"
# ============================================================
